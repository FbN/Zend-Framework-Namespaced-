    ...
    public function __construct($commentOrReflector)
    {
        if ($commentOrReflector instanceof \Reflector) {
            ...
            if (!method_exists($commentOrReflector, 'getDocComment')) {
                ...
                throw new Reflection\Exception('Reflector must contain method "getDocComment"');
            }
            ...
            require_once 'Zend/Reflection/Exception.php';
            throw new Reflection\Exception(get_class($this) . ' must have a (string) DocComment or a Reflector in the constructor');
        }

        if ($docComment == '') {
            require_once 'Zend/Reflection/Exception.php';
            throw new Reflection\Exception('DocComment cannot be empty');
        }
    ...
    protected function _parse()
    {
        ...
        while (($newlinePos = strpos($parsedDocComment, "\n")) !== false) {
            ...
            if ((strpos($line, '@') === 0) && (preg_match('#^(@\w+.*?)(\n)(?:@|\r?\n|$)#s', $parsedDocComment, $matches))) {
                $this->_tags[] = Tag\Tag::factory($matches[1]);
