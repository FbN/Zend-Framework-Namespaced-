               ...
               ResourceBootstrapper
{
    ...
    public function registerPluginResource($resource, $options = null)
    {
        ...
        if (!is_string($resource)) {
            throw new Exception('Invalid resource provided to ' . __METHOD__);
        }
    ...
    public function unregisterPluginResource($resource)
    {
        ...
        if (!is_string($resource)) {
            throw new Exception('Unknown resource type provided to ' . __METHOD__);
        }
    ...
    public function getPluginResource($resource)
    {
        if (array_key_exists(strtolower($resource), $this->_pluginResources)) {
            $resource = strtolower($resource);
            if (!$this->_pluginResources[$resource] instanceof Resource\Resource) {
                $resourceName = $this->_loadPluginResource($resource, $this->_pluginResources[$resource]);
                if (!$resourceName) {
                    throw new Exception(sprintf('Unable to resolve plugin "%s"; no corresponding plugin with that name', $resource));
                }
        ...
        foreach ($this->_pluginResources as $plugin => $spec) {
            if ($spec instanceof Resource\Resource) {
            ...
            if (class_exists($plugin)) { //@SEE ZF-7550
                ...
                $pluginName = $this->\\resolvePluginResourceName($instance);
                unset($this->_pluginResources[$plugin]);
    ...
    {
        if (($application instanceof \Zend\Application\Application)
            || ($application instanceof Bootstrapper)
        ) {
            if ($application === $this) {
                throw new Exception('Cannot set application to same object; creates recursion');
            }
            $this->_application = $application;
        } else {
            throw new Exception('Invalid application provided to bootstrap constructor (received "' . get_class($application) . '" instance)');
        }
        return $this;
    }
    ...
    public function setContainer($container)
    {
        if (!is_object($container)) {
            throw new Exception('Resource containers must be objects');
        }
    ...
    protected function _bootstrap($resource = null)
    {
        if (null === $resource) {
            ...
            throw new Exception('Invalid argument passed to ' . __METHOD__);
        }
    }
    ...
    protected function _executeResource($resource)
    {
        ...
        throw new Exception('Resource matching "' . $resource . '" not found');
    }
    ...
    protected function _loadPluginResource($resource, $options)
    {
        ...
        unset($this->\\pluginResources[$resource]);
