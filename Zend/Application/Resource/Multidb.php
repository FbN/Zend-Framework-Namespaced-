 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Multidb extends ResourceAbstract
{
    /**
    ...
    {
        Table\Table::setDefaultAdapter($adapter);
   ...
   /**
    ...
    protected function _setDefaultMetadataCache($cache)
    {
        ...
        if (is_string($cache)) {
            ...
            if ($bootstrap instanceof \Zend\Application\Bootstrap\ResourceBootstrapper &&
        ...
        } else if ($cache instanceof Cache\Core) {
            $metadataCache = $cache;
        }

        if ($metadataCache instanceof Cache\Core) {
            Table\Table::setDefaultMetadataCache($metadataCache);
        }
