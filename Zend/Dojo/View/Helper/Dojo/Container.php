...
class Container
{
    ...
    public function requireModule($modules)
    {
        if (!is_string($modules) && !is_array($modules)) {
            ...
            throw new View\Exception('Invalid module name specified; must be a string or an array of strings');
        }
        ...
        foreach ($modules as $mod) {
            if (!preg_match('/^[a-z][a-z0-9._-]+$/i', $mod)) {
                require_once 'Zend/Dojo/View/Exception.php';
                throw new View\Exception(sprintf('Module name specified, "%s", contains invalid characters', (string) $mod));
            }
    ...
    public function addStylesheetModule($module)
    {
        if (!preg_match('/^[a-z0-9]+\.[a-z0-9_-]+(\.[a-z0-9_-]+)*$/i', $module)) {
            require_once 'Zend/Dojo/View/Exception.php';
            throw new View\Exception('Invalid stylesheet module specified');
        }
    ...
    public function onLoadCaptureStart()
    {
        if ($this->_captureLock) {
            require_once 'Zend/Dojo/View/Exception.php';
            throw new View\Exception('Cannot nest onLoad captures');
        }
    ...
    public function addDijit($id, array $params)
    {
        if (array_key_exists($id, $this->_dijits)) {
            require_once 'Zend/Dojo/View/Exception.php';
            throw new View\Exception(sprintf('Duplicate dijit with id "%s" already registered', $id));
        }
    ...
    public function dijitsToJson()
    {
        ...
        return Json\Json::encode($this->getDijits(), false, array('enableJsonExprFinder' => true));
    }
...
EOJ;
    ...
    public function javascriptCaptureStart()
    {
        if ($this->_captureLock) {
            require_once 'Zend/Dojo/View/Exception.php';
            throw new View\Exception('Cannot nest captures');
        }
    ...
    protected function _renderDjConfig()
    {
        ...
        $scriptTag = '<script type="text/javascript">' . PHP_EOL
                   ...
                   . '    var djConfig = ' . Json\Json::encode($djConfigValues) . ';' . PHP_EOL
                   . (($this->_isXhtml) ? '//]]>' : '//-->') . PHP_EOL
                   . '</script>';
