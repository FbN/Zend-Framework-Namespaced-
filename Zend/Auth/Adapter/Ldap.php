...
class Ldap implements AdapterInterface
{
     ...
     * @return Zend_Auth_Adapter_Ldap Provides a fluent interface
     */
    public function setLdap(Ldap\Ldap $ldap)
    {
        $this->_ldap = $ldap;
    ...
    public function authenticate()
    {
        ...
        if (!$username) {
            $code = Auth\Result::FAILURE_IDENTITY_NOT_FOUND;
            $messages[0] = 'A username is required';
            return new Auth\Result($code, '', $messages);
        }
        if (!$password) {
            /* A password is required because some servers will
             * treat an empty password as an anonymous bind.
             */
            $code = Auth\Result::FAILURE_CREDENTIAL_INVALID;
        ...
        foreach ($this->_options as $name => $options) {
            ...
            try {
                ...
                if ($groupResult === true) {
                    ...
                    return new Auth\Result(Auth\Result::SUCCESS, $canonicalName, $messages);
                } else {
                ...
                if ($err == Ldap\Exception::LDAP_X_DOMAIN_MISMATCH) {
                     ...
                     */
                    continue;
                } else if ($err == Ldap\Exception::LDAP_NO_SUCH_OBJECT) {
                    $code = Auth\Result::FAILURE_IDENTITY_NOT_FOUND;
                    $messages[0] = "Account not found: $username";
                    $failedAuthorities[$dname] = $zle->getMessage();
                } else if ($err == Ldap\Exception::LDAP_INVALID_CREDENTIALS) {
                    $code = Auth\Result::FAILURE_CREDENTIAL_INVALID;
        ...
        return new Auth\Result($code, $username, $messages);
    }
     ...
     */
    protected function _prepareOptions(Ldap\Ldap $ldap, array $options)
    {
        $adapterOptions = array(
            'group'       => null,
            'groupDn'     => $ldap->getBaseDn(),
            'groupScope'  => Ldap\Ldap::SEARCH_SCOPE_SUB,
            'groupAttr'   => 'cn',
        ...
        foreach ($adapterOptions as $key => $value) {
            if (array_key_exists($key, $options)) {
                ...
                switch ($key) {
                    case 'groupScope':
                        $value = (int)$value;
                        if (in_array($value, array(Ldap\Ldap::SEARCH_SCOPE_BASE,
                                Ldap\Ldap::SEARCH_SCOPE_ONE, Ldap\Ldap::SEARCH_SCOPE_SUB), true)) {
     ...
     * @return string|true
     */
    protected function _checkGroupMembership(Ldap\Ldap $ldap, $canonicalName, $dn, array $adapterOptions)
    {
        ...
        $groupName = Filter\Filter::equals($adapterOptions['groupAttr'], $adapterOptions['group']);
        $membership = Filter\Filter::equals($adapterOptions['memberAttr'], $user);
        $group = Filter\Filter::andFilter($groupName, $membership);
