        ...
        while ($line = trim(fgets($fileHandle))) {
            if (substr($line, 0, $idLength) === $id) {
                if ($this->_secureStringCompare(substr($line, -32), md5("$this->_username:$this->_realm:$this->_password"))) {
                    $result['code'] = Auth\Result::SUCCESS;
                } else {
                    $result['code'] = Auth\Result::FAILURE_CREDENTIAL_INVALID;
                    $result['messages'][] = 'Password incorrect';
                }
                return new Auth\Result($result['code'], $result['identity'], $result['messages']);
            }
        }

        $result['code'] = Auth\Result::FAILURE_IDENTITY_NOT_FOUND;
