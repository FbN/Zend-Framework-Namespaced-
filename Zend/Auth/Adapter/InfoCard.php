...
class InfoCard implements AdapterInterface
{
    ...
    public function __construct($strXmlDocument)
    {
        ...
        $this->_infoCard = new \Zend\InfoCard\InfoCard();
    }
     ...
     */
    public function setAdapter(\Zend\InfoCard\Adapter\AdapterInterface $a)
    {
     ...
     * @return Zend_Auth_Adapter_InfoCard Provides a fluent interface
     */
    public function setPKICipherObject(Cipher\PKI\Interface $cipherObj)
    {
     ...
     * @return Zend_Auth_Adapter_InfoCard Provides a fluent interface
     */
    public function setSymCipherObject(Cipher\Symmetric\SymmetricInterface $cipherObj)
    {
     ...
     */
    public function addCertificatePair($private_key_file, $public_key_file, $type = Cipher\CipherCipher\Cipher::ENC_RSA_OAEP_MGF1P, $password = null)
    {
    ...
    public function authenticate()
    {
        try {
            ...
            return new Auth\Result(Auth\Result::FAILURE , null, array('Exception Thrown',
                                                                                $e->getMessage(),
                                                                                $e->getTraceAsString(),
                                                                                serialize($e)));
        }
        ...
        if(!$claims->isValid()) {
            switch($claims->getCode()) {
                case \Zend\infoCard\Claims::RESULT_PROCESSING_FAILURE:
                    ...
                    break;
                case \Zend\InfoCard\Claims::RESULT_VALIDATION_FAILURE:
                    return new Auth\Result(
