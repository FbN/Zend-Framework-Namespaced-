    ...
    extends DocumentService\Adapter\AbstractAdapter
{
    ...
    public function __construct($options = array()) 
    {
        ...
        if (!is_array($options)) {
            throw new DocumentService\Exception('Invalid options provided to constructor');
        }

        $this->_simpleDb = new Amazon\SimpleDb\SimpleDb(
    ...
    public function createCollection($name, $options = null) 
    {
        try {
            ...
            throw new DocumentService\Exception('Error on domain creation: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function fetchDocument($collectionName, $documentId, $options = null)
    {
        try {
            ...
            throw new DocumentService\Exception('Error on fetching document: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    protected function _makeAttributes($name, $attributes)
    {
        ...
        foreach ($attributes as $key => $attr) {
            $result[] = new Amazon\SimpleDb\Attribute($name, $key, $attr);
        }
        return $result;
    }
    ...
    protected function _getDocumentFromArray($document)
    {
        if (!isset($document[DocumentService\Document::KEY_FIELD])) {
            if (isset($document[self::ITEM_NAME])) {
                ...
                throw new DocumentService\Exception('Fields array should contain the key field '.DocumentService\Document::KEY_FIELD);
            }
        } else {
     ...
     * @return Zend_Cloud_DocumentService_DocumentSet
     */
    protected function _getDocumentSetFromResultSet(Amazon\SimpleDb\Page $resultSet, $returnDocs = true)
    {
        $docs = array();
