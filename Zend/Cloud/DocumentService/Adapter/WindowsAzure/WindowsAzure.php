...
use Zend\Cloud\DocumentService;
 ...
 */
class WindowsAzure 
    extends DocumentService\Adapter\AbstractAdapter
{
    /*
    ...
    const DEFAULT_HOST          = Storage\Storage::URL_CLOUD_TABLE;
    ...
    public function __construct($options = array()) 
    {
        ...
        if (!is_array($options)) {
            throw new DocumentService\Exception('Invalid options provided');
        }
        ...
        if (! isset($options[self::ACCOUNT_NAME])) {
            throw new DocumentService\Exception('No Windows Azure account name provided.');
        }

        if (! isset($options[self::ACCOUNT_KEY])) {
            throw new DocumentService\Exception('No Windows Azure account key provided.');
        }

        // TODO: support $usePathStyleUri and $retryPolicy
        try {
            $this->_storageClient = new Storage\Table(
            ...
            throw new DocumentService\Exception('Error on document service creation: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function createCollection($name, $options = null) 
    {
        if (!preg_match('/^[A-Za-z][A-Za-z0-9]{2,}$/', $name)) {
            throw new DocumentService\Exception('Invalid collection name; Windows Azure collection names must consist of alphanumeric characters only, and be at least 3 characters long');
        }
        try {
            $this->_storageClient->createTable($name);
        } catch(WindowsAzure\Exception $e) {
            if (strpos($e->getMessage(), "table specified already exists") === false) {
                throw new DocumentService\Exception('Error on collection creation: '.$e->getMessage(), $e->getCode(), $e);
            }
        }
        return true;
    }
    ...
    public function deleteCollection($name, $options = null) 
    {
        try {
            ...
            if (strpos($e->getMessage(), "does not exist") === false) {
                throw new DocumentService\Exception('Error on collection deletion: '.$e->getMessage(), $e->getCode(), $e);
            }
        }
        return true;
    }
    ...
    public function listCollections($options = null) 
    {
        try {
            ...
            throw new DocumentService\Exception('Error on collection list: '.$e->getMessage(), $e->getCode(), $e);
        }
    ...
    {
        $key = null;
        if (!isset($document[DocumentService\Document::KEY_FIELD])) {
            ...
            $key = $document[DocumentService\Document::KEY_FIELD];
            unset($document[DocumentService\Document::KEY_FIELD]);
        }
    ...
    public function insertDocument($collectionName, $document, $options = null)
    {
        ...
        if (!$document instanceof DocumentService\Document) {
            throw new DocumentService\Exception('Invalid document supplied');
        }
        ...
        try {
            ...
            $entity = new Storage\DynamicTableEntity($key[0], $key[1]);
            $entity->setAzureValues($document->getFields(), true);
            $this->_storageClient->insertEntity($collectionName, $entity);
        } catch(WindowsAzure\Exception $e) {
            throw new DocumentService\Exception('Error on document insertion: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function replaceDocument($collectionName, $document, $options = null)
    {
        ...
        try {
            ...
            throw new DocumentService\Exception('Error on document replace: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    {
        if (null === $fieldset && $documentId instanceof DocumentService\Document) {
            $fieldset   = $documentId->getFields();
            $documentId = $documentId->getId();
        } elseif ($fieldset instanceof DocumentService\Document) {
            if ($documentId == null) {
        ...
        try {
            $entity = new Storage\DynamicTableEntity($documentId[0], $documentId[1]);
            ...
            throw new DocumentService\Exception('Error on document update: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    {
        if ($documentId instanceof DocumentService\Document) {
            $documentId = $documentId->getId();
        }
        ...
        try {
            ...
            if (strpos($e->getMessage(), "does not exist") === false) {
                throw new DocumentService\Exception('Error on document deletion: '.$e->getMessage(), $e->getCode(), $e);
            }
        }
    }
    ...
    public function fetchDocument($collectionName, $documentId, $options = null)
    {
        ...
        try {
            ...
            $documentClass = $this->getDocumentClass();
            return new $documentClass($this->\\resolveAttributes($entity), array($entity->getPartitionKey(), $entity->getRowKey()));
        } catch (WindowsAzure\Exception $e) {
            if (strpos($e->getMessage(), "does not exist") !== false) {
                return false;
            }
            throw new DocumentService\Exception('Error on document fetch: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
        ...
        try {
            ...
            foreach ($entities as $entity) {
                $resultSet[] = new $documentClass(
                    $this->\\resolveAttributes($entity),
                    array($entity->getPartitionKey(), $entity->getRowKey())
                );
            }
        } catch(WindowsAzure\Exception $e) {
            throw new DocumentService\Exception('Error on document query: '.$e->getMessage(), $e->getCode(), $e);
        }
    ...
    public function select($fields = null)
    {
        ...
        if (!class_exists($queryClass)) {
            ...
            \Zend\Loader\Loader::loadClass($queryClass);
        }

        $query = new $queryClass();
        $defaultClass = \self::DEFAULT_QUERY_CLASS;
        if (!$query instanceof \$defaultClass) {
            throw new DocumentService\Exception('Query class must extend ' . self::DEFAULT_QUERY_CLASS);
        }
     ...
     * @return array
     */
    protected function _resolveAttributes(Storage\TableEntity $entity)
    {
        $result = array();
    ...
    protected function _validateKey($key)
    {
        if (preg_match('@[/#?' . preg_quote('\\') . ']@', $key)) {
            throw new DocumentService\Exception('Invalid partition or row key provided; must not contain /, \\,  #, or ? characters');
        }
    }
    ...
    {
        if (2 != count($key)) {
            throw new DocumentService\Exception('Invalid document key provided; must contain exactly two elements: a PartitionKey and a RowKey');
        }
        foreach ($key as $k) {
            $this->_validateKey($k);
        }
    ...
    protected function _validateDocumentId($documentId, $collectionName = false)
    {
        ...
        if (!is_string($documentId)) {
            throw new DocumentService\Exception('Invalid document identifier; must be a string or an array');
        }
        ...
        throw new DocumentService\Exception('Cannot determine partition name; invalid document identifier');
    }
    ...
    public function _validateFields($document)
    {
        if ($document instanceof DocumentService\Document) {
            ...
            throw new DocumentService\Exception('Cannot inspect fields; invalid type provided');
        }
    ...
    public function _validateFieldKey($key)
    {
        if (!preg_match('/^[_A-Za-z][-._A-Za-z0-9]*$/', $key)) {
            throw new DocumentService\Exception('Field keys must conform to XML names (^[_A-Za-z][-._A-Za-z0-9]*$); key "' . $key . '" does not match');
        }
    }
}
