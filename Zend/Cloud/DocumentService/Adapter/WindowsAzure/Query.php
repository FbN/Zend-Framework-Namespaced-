 ...
 */
class Query
    implements \Zend\Cloud\DocumentService\QueryAdapter
{
    /**
    ...
    {
        if (!$select instanceof Storage\TableEntityQuery) {
            require_once 'Zend/Service/WindowsAzure/Storage/TableEntityQuery.php';
            $select = new Storage\TableEntityQuery();
        }
    ...
    public function whereId($value)
    {
        if (!is_array($value)) {
            ...
            throw new \Zend\Cloud\DocumentService\Exception('Invalid document key');
        }
    ...
    public function order($sort, $direction = 'asc')
    {
        ...
        throw new \Zend\Cloud\OperationNotAvailableException('No support for sorting for Azure yet');
    }
