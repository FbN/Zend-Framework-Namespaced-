    ...
    implements Adapter
{
    ...
    const DEFAULT_HOST = WindowsAzure\Storage\Storage::URL_CLOUD_BLOB;
    ...
    public function __construct($options = array())
    {
        ...
        if (!isset($options[self::ACCOUNT_NAME])) {
            throw new StorageService\Exception('No Windows Azure account name provided.');
        }
        if (!isset($options[self::ACCOUNT_KEY])) {
            throw new StorageService\Exception('No Windows Azure account key provided.');
        }
            
        $this->_storageClient = new WindowsAzure\Storage\Blob\Blob($host,
    ...
    public function storeMetadata($destinationPath, $metadata, $options = null)
    {
        try    {
            ...
            if (strpos($e->getMessage(), "could not be accessed") === false) {
                throw new StorageService\Exception('Error on store metadata: '.$e->getMessage(), $e->getCode(), $e);
            }
        }
    }
    ...
    public function deleteMetadata($path, $options = null)
    {
        try {
            ...
            if (strpos($e->getMessage(), "could not be accessed") === false) {
                throw new StorageService\Exception('Error on delete metadata: '.$e->getMessage(), $e->getCode(), $e);
            }
        }
    }
