    ...
    implements Adapter
{
    ...
    public function __construct($options = array()) 
    {
        ...
        if (!is_array($options)) {
            throw new StorageService\Exception('Invalid options provided');
        }

        if (!isset($options[self::AWS_ACCESS_KEY]) || !isset($options[self::AWS_SECRET_KEY])) {
            throw new StorageService\Exception('AWS keys not specified!');
        }

        try {
            $this->_s3 = new S3\S3($options[self::AWS_ACCESS_KEY],
                                                $options[self::AWS_SECRET_KEY]);
        } catch (S3\Exception  $e) { 
            throw new StorageService\Exception('Error on create: '.$e->getMessage(), $e->getCode(), $e);
        }
    ...
    public function fetchItem($path, $options = array()) 
    {
        ...
        try {
            ...
            throw new StorageService\Exception('Error on fetch: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function storeItem($destinationPath, $data, $options = array()) 
    {
        try {
            ...
            throw new StorageService\Exception('Error on store: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    {    
        try {
            ...
            throw new StorageService\Exception('Error on delete: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function copyItem($sourcePath, $destinationPath, $options = array()) 
    {
        try {
            ...
            throw new StorageService\Exception('Error on copy: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function moveItem($sourcePath, $destinationPath, $options = array()) 
    {
        try {
            ...
            throw new StorageService\Exception('Error on move: '.$e->getMessage(), $e->getCode(), $e);
        }
     }
    ...
    public function renameItem($path, $name, $options = null) 
    {
        ...
        throw new Cloud\OperationNotAvailableException('Rename not implemented');
    }
    ...
    public function listItems($path, $options = null) 
    {
        try {
            ...
            throw new StorageService\Exception('Error on list: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    {
        require_once 'Zend/Cloud/OperationNotAvailableException.php';
        throw new Cloud\OperationNotAvailableException('Storing separate metadata is not supported, use storeItem() with \'metadata\' option key');
    }
    ...
    {
        require_once 'Zend/Cloud/OperationNotAvailableException.php';
        throw new Cloud\OperationNotAvailableException('Deleting metadata not supported');
    }
    ...
    protected function _getFullPath($path, $options) 
    {
        if (isset($options[self::BUCKET_NAME])) {
            ...
            require_once 'Zend/Cloud/StorageService/Exception.php';
            throw new StorageService\Exception('Bucket name must be specified for S3 adapter.');
        }

        if (isset($options[self::BUCKET_AS_DOMAIN])) {
            // TODO: support bucket domain names
            require_once 'Zend/Cloud/StorageService/Exception.php';
            throw new StorageService\Exception('The S3 adapter does not currently support buckets in domain names.');
        }
