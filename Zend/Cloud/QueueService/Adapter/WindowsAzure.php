    ...
    extends AbstractAdapter
{
    ...
    const DEFAULT_HOST = Storage\Storage::URL_CLOUD_QUEUE;
    ...
    public function __construct($options = array())
    {
        ...
        if (! isset($options[self::ACCOUNT_NAME])) {
            throw new \Zend\Cloud\Storage\Exception('No Windows Azure account name provided.');
        }
        if (! isset($options[self::ACCOUNT_KEY])) {
            throw new \Zend\Cloud\Storage\Exception('No Windows Azure account key provided.');
        }
        try {
            // TODO: support $usePathStyleUri and $retryPolicy
            $this->_storageClient = new Storage\Queue(
    ...
    {
        try {
            if ($queueId instanceof Storage\QueueInstance) {
    ...
    public function deleteMessage($queueId, $message, $options = null)
    {
        try {
            ...
            if ($message instanceof Storage\QueueMessage) {
                return $this->_storageClient->deleteMessage($queueId, $message);
            } else {
                throw new QueueService\Exception('Cannot delete the message: message object required');
            }
        } catch (WindowsAzure\Exception $e) {
    ...
    public function peekMessages($queueId, $num = 1, $options = null)
    {
        try {
            ...
            throw new QueueService\Exception('Error on peeking messages: '.$e->getMessage(), $e->getCode(), $e);
        }
   }
