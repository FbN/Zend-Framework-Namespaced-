    ...
    extends AbstractAdapter
{
    ...
    public function __construct ($options = array())
    {
        ...
        if (!is_array($options)) {
            throw new QueueService\Exception('Invalid options provided');
        }
        ...
        if (!isset($options[self::ADAPTER])) {
            throw new QueueService\Exception('No Zend_Queue adapter provided');
        } else {
            $adapter = $options[self::ADAPTER];
            unset($options[self::ADAPTER]);
        }
        try {
            $this->_queue = new Queue\Queue($adapter, $options);
        } catch (Queue\Exception $e) {
            throw new QueueService\Exception('Error on create: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    {
        try {
            $this->_queues[$name] = $this->_queue->createQueue($name, isset($options[Queue\Queue::TIMEOUT])?$options[Queue\Queue::TIMEOUT]:null);
            return $name;
        } catch (Queue\Exception $e) {
            throw new QueueService\Exception('Error on queue creation: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function deleteQueue($queueId, $options = null)
    {
        ...
        try {
            ...
            throw new QueueService\Exception('Error on queue deletion: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function listQueues($options = null)
    {
        try {
            ...
            throw new QueueService\Exception('Error on listing queues: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function fetchQueueMetadata($queueId, $options = null)
    {
        ...
        try {
            ...
            throw new QueueService\Exception('Error on fetching queue metadata: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    {
        if (!isset($this->_queues[$queueId])) {
            throw new QueueService\Exception("No such queue: $queueId");
        }
        try {
            return $this->_queues[$queueId]->setOptions($metadata);
        } catch (Queue\Exception $e) {
            throw new QueueService\Exception('Error on setting queue metadata: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function sendMessage($queueId, $message, $options = null)
    {
        ...
        try {
            ...
            throw new QueueService\Exception('Error on sending message: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function receiveMessages($queueId, $max = 1, $options = null)
    {
        ...
        try {
            $res = $this->_queues[$queueId]->receive($max, isset($options[Queue\Queue::TIMEOUT])?$options[Queue\Queue::TIMEOUT]:null);
            ...
            throw new QueueService\Exception('Error on recieving messages: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function deleteMessage($queueId, $message, $options = null)
    {
        ...
        try {
            if ($message instanceof QueueService\Message) {
                $message = $message->getMessage();
            }
            if (!($message instanceof Queue\Message\Message)) {
                throw new QueueService\Exception('Cannot delete the message: Zend_Queue_Message object required');
            }
            
            return $this->_queues[$queueId]->deleteMessage($message);
        } catch (Queue\Exception $e) {
            throw new QueueService\Exception('Error on deleting a message: '.$e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function peekMessages($queueId, $num = 1, $options = null)
    {
        ...
        throw new \Zend\Cloud\OperationNotAvailableException('ZendQueue doesn\'t currently support message peeking');
    }
