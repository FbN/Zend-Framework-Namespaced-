 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Logical extends AbstractFilter
{
    const TYPE_AND = '&';
    const TYPE_OR  = '|';
    ...
    protected function __construct(array $subfilters, $symbol)
    {
        foreach ($subfilters as $key => $s) {
            if (is_string($s)) $subfilters[$key] = new String($s);
            else if (!($s instanceof AbstractFilter)) {
                /**
                 * @see Zend_Ldap_Filter_Exception
                 */
                require_once 'Zend/Ldap/Filter/Exception.php';
                throw new Exception('Only strings or Zend_Ldap_Filter_Abstract allowed.');
            }
        }
