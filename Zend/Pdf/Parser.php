...
class Parser
{
    ...
    private function _loadXRefTable($offset)
    {
        ...
        $refTable = new Element\Reference\Table();
        require_once 'Zend/Pdf/Element/Reference/Context.php';
        $context  = new Element\Reference\Context($this->_stringParser, $refTable);
        ...
        if ($nextLexeme == 'xref') {
            ...
            while ( ($nextLexeme = $this->_stringParser->readLexeme()) != 'trailer' ) {
                if (!ctype_digit($nextLexeme)) {
                    ...
                    throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. Cross-reference table subheader values must contain only digits.', $this->_stringParser->offset-strlen($nextLexeme)));
                }
                ...
                if (!ctype_digit($refCount)) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. Cross-reference table subheader values must contain only digits.', $this->_stringParser->offset-strlen($refCount)));
                }
                ...
                while ($refCount > 0) {
                    ...
                    if (!ctype_digit($objectOffset)) {
                        require_once 'Zend/Pdf/Exception.php';
                        throw new Exception(sprintf('PDF file cross-reference table syntax error. Offset - 0x%X. Offset must contain only digits.', $this->_stringParser->offset));
                    }
                    ...
                    if (strpos("\x00\t\n\f\r ", $this->_stringParser->data[$this->_stringParser->offset]) === false) {
                        require_once 'Zend/Pdf/Exception.php';
                        throw new Exception(sprintf('PDF file cross-reference table syntax error. Offset - 0x%X. Value separator must be white space.', $this->_stringParser->offset));
                    }
                    ...
                    if ( !StringParser::isWhiteSpace(ord( $this->_stringParser->data[$this->_stringParser->offset] )) ) {
                        require_once 'Zend/Pdf/Exception.php';
                        throw new Exception(sprintf('PDF file cross-reference table syntax error. Offset - 0x%X. Value separator must be white space.', $this->_stringParser->offset));
                    }
                    $this->_stringParser->offset++;
            ...
            if (!$trailerDict instanceof Element\Dictionary) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X.  Dictionary expected after \'trailer\' keyword.', $trailerDictOffset));
            }
        } else {
            $xrefStream = $this->_stringParser->getObject($offset, $context);

            if (!$xrefStream instanceof Element\Object\Stream) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X.  Cross-reference stream expected.', $offset));
            }

            $trailerDict = $xrefStream->dictionary;
            if ($trailerDict->Type->value != 'XRef') {
                require_once 'Zend/Pdf/Exception.php';
                throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X.  Cross-reference stream object must have /Type property assigned to /XRef.', $offset));
            }
            if ($trailerDict->W === null  || $trailerDict->W->getType() != Element\Element::TYPE_ARRAY) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. Cross reference stream dictionary doesn\'t have W entry or it\'s not an array.', $offset));
            }
            ...
            if ($entryField2Size == 0 || $entryField3Size == 0) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. Wrong W dictionary entry. Only type field of stream entries has default value and could be zero length.', $offset));
            }

            $xrefStreamData = $xrefStream->value;

            if ($trailerDict->Index !== null) {
                if ($trailerDict->Index->getType() != Element\Element::TYPE_ARRAY) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. Cross reference stream dictionary Index entry must be an array.', $offset));
                }
            ...
            for ($count = 0; $count < $sections; $count++) {
                ...
                for ($count2 = 0; $count2 < $entries; $count2++) {
                    if ($entryField1Size == 0) {
                        ...
                        $type = StringParser::parseIntFromStream($xrefStreamData, $streamOffset, $entryField1Size);
                    ...
                    if ($entryField2Size == 1) { // Optimyze one-byte field case
                        ...
                        $field2 = StringParser::parseIntFromStream($xrefStreamData, $streamOffset, $entryField2Size);
                    ...
                    if ($entryField3Size == 1) { // Optimyze one-byte field case
                        ...
                        $field3 = StringParser::parseIntFromStream($xrefStreamData, $streamOffset, $entryField3Size);
            ...
            // "$entries\n";
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception('Cross-reference streams are not supported yet.');
        }


        require_once 'Zend/Pdf/Trailer/Keeper.php';
        $trailerObj = new Trailer\Keeper($trailerDict, $context);
        if ($trailerDict->Prev instanceof Element\Numeric ||
            $trailerDict->Prev instanceof Element\Reference\Reference ) {
         ...
         */
        $trailerObj->Prev = new Element\Numeric($offset);
     ...
     */
    public function __construct($source, ElementFactory\ElementFactoryInterface $factory, $load)
    {
        if ($load) {
            if (($pdfFile = @fopen($source, 'rb')) === false ) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Exception( "Can not open '$source' file for reading." );
            }
            ...
            while ($byteCount > 0 && !feof($pdfFile)) {
                ...
                if ($nextBlock === false) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Exception( "Error occured while '$source' file reading." );
                }
            ...
            $this->_stringParser = new StringParser($data, $factory);
        } else {
            $this->_stringParser = new StringParser($source, $factory);
        }
        ...
        if (version_compare($pdfVersion, '0.9',  '<')  ||
             ...
             */
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('Unsupported PDF version. Zend_Pdf supports PDF 1.0-1.4. Current version - \'%f\'', $pdfVersion));
        }
        ...
        if ($this->_stringParser->offset === false ||
            ...
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception('Pdf file syntax error. End-of-fle marker expected at the end of file.');
        }
         ...
         */
        while (StringParser::isWhiteSpace( ord($this->_stringParser->data[$this->_stringParser->offset]) )&&
        ...
        if ($nextLexeme != 'startxref') {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('Pdf file syntax error. \'startxref\' keyword expected. Offset - 0x%X.', $this->_stringParser->offset-strlen($nextLexeme)));
        }

        $startXref = $this->_stringParser->readLexeme();
        if (!ctype_digit($startXref)) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('Pdf file syntax error. Cross-reference table offset must contain only digits. Offset - 0x%X.', $this->_stringParser->offset-strlen($nextLexeme)));
        }
