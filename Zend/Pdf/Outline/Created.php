...
class Created extends Outline
{
    ...
    public function setTarget($target = null)
    {
        ...
        if ($target === null  ||  $target instanceof Pdf\Target) {
            $this->_target = $target;
        } else {
                                                 ...
                                                 \SplObjectStorage $processedOutlines = null)
    {
        ...
        $outlineDictionary = $factory->newObject(new Element\Dictionary());

        $outlineDictionary->Title = new Element\String\String($this->getTitle());

        $target = $this->getTarget();
        if ($target === null) {
            // Do nothing
        } else if ($target instanceof Pdf\Destination\Destination) {
            $outlineDictionary->Dest = $target->getResource();
        } else if ($target instanceof Pdf\Action\Action) {
            $outlineDictionary->A    = $target->getResource();
        } else {
            require_once 'Zend/Pdf/Exception.php';
            throw new Pdf\Exception('Outline target has to be Zend_Pdf_Destination, Zend_Pdf_Action object or null');
        }
        ...
        if ($color !== null) {
            ...
            $outlineDictionary->C = new Element\Array($colorComponentElements);
        }

        if ($this->isItalic()  ||  $this->isBold()) {
            $outlineDictionary->F = new Element\Numeric(($this->isItalic()? 1 : 0)  |   // Bit 1 - Italic
        ...
        if (count($this->childOutlines) != 0) {
            $outlineDictionary->Count = new Element\Numeric(($this->isOpen()? 1 : -1)*count($this->childOutlines));
        }
