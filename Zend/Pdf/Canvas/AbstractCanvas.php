...
abstract class AbstractCanvas implements CanvasInterface
{
     ...
     * @return string
     */
    abstract protected function _attachResource($type, Pdf\Resource\Resource $resource);
     ...
     */
    public function drawCanvas(CanvasInterface $canvas, $x1, $y1, $x2 = null, $y2 = null)
    {
        $this->saveGS();
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function setFillColor(Color\Color $color)
    {
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function setLineColor(Color\Color $color)
    {
        $this->_addProcSet('PDF');
    ...
    {
        $this->_addProcSet('PDF');
        $widthObj = new Element\Numeric($width);
    ...
    public function setLineDashingPattern($pattern, $phase = 0)
    {
        ...
        if ($pattern === Pdf\Page::LINE_DASHING_SOLID) {
            $pattern = array();
            $phase   = 0;
        }

        $dashPattern  = new Pdf\Element\Array();
        $phaseEleemnt = new Element\Numeric($phase);

        foreach ($pattern as $dashItem) {
            $dashElement = new Element\Numeric($dashItem);
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function setFont(Pdf\Resource\Font\Font $font, $fontSize)
    {
        ...
        $fontNameObj = new Element\Name($fontName);
        $fontSizeObj = new Element\Numeric($fontSize);
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function setStyle(Pdf\Style $style)
    {
        $this->_addProcSet('Text');
        $this->_addProcSet('PDF');
    ...
    public function restoreGS()
    {
        if ($this->_saveCount-- <= 0) {
            ...
            throw new Pdf\Exception('Restoring graphics state which is not saved');
        }
        $this->_contents .= " Q\n";
    ...
    public function setAlpha($alpha, $mode = 'Normal')
    {
        ...
        $graphicsState = new Pdf\Resource\GraphicsState();

        $graphicsState->setAlpha($alpha, $mode);
        $gStateName = $this->_attachResource('ExtGState', $graphicsState);

        $gStateNameObject = new Element\Name($gStateName);
    ...
    public function clipEllipse($x1, $y1, $x2, $y2, $startAngle = null, $endAngle = null)
    {
        ...
        $xC = new Element\Numeric($x);
        $yC = new Element\Numeric($y);
        ...
        if ($startAngle !== null) {
            ...
            for($count = 0; $count <= $clipSectors; $count++) {
                ...
                $pX = new Element\Numeric($x + cos($pAngle)*$clipRadius);
                $pY = new Element\Numeric($y + sin($pAngle)*$clipRadius);
        ...
        $xLeft  = new Element\Numeric($x1);
        $xRight = new Element\Numeric($x2);
        $yUp    = new Element\Numeric($y2);
        $yDown  = new Element\Numeric($y1);

        $xDelta  = 2*(M_SQRT2 - 1)*($x2 - $x1)/3.;
        $yDelta  = 2*(M_SQRT2 - 1)*($y2 - $y1)/3.;
        $xr = new Element\Numeric($x + $xDelta);
        $xl = new Element\Numeric($x - $xDelta);
        $yu = new Element\Numeric($y + $yDelta);
        $yd = new Element\Numeric($y - $yDelta);
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function clipPolygon($x, $y, $fillMethod = Pdf\PagePdf\Page::FILL_METHOD_NON_ZERO_WINDING)
    {
        $this->_addProcSet('PDF');

        $firstPoint = true;
        foreach ($x as $id => $xVal) {
            $xObj = new Element\Numeric($xVal);
            $yObj = new Element\Numeric($y[$id]);
        ...
        if ($fillMethod == Pdf\Page::FILL_METHOD_NON_ZERO_WINDING) {
            $this->_contents .= " h\n W\nn\n";
        } else {
    ...
    public function clipRectangle($x1, $y1, $x2, $y2)
    {
        ...
        $x1Obj      = new Element\Numeric($x1);
        $y1Obj      = new Element\Numeric($y1);
        $widthObj   = new Element\Numeric($x2 - $x1);
        $height2Obj = new Element\Numeric($y2 - $y1);
...
// ------------------------------------------------------------------------------------------
    ...
    public function drawEllipse($x1, $y1, $x2, $y2, $param5 = null, $param6 = null, $param7 = null)
    {
        if ($param5 === null) {
            // drawEllipse($x1, $y1, $x2, $y2);
            $startAngle = null;
            $fillType = Pdf\Page::SHAPE_DRAW_FILL_AND_STROKE;
        } else if ($param6 === null) {
            // drawEllipse($x1, $y1, $x2, $y2, $fillType);
            $startAngle = null;
            $fillType = $param5;
        ...
        switch ($fillType) {
            case Pdf\Page::SHAPE_DRAW_FILL_AND_STROKE:
                $this->_contents .= " B*\n";
                break;
            case Pdf\Page::SHAPE_DRAW_FILL:
                $this->_contents .= " f*\n";
                break;
            case Pdf\Page::SHAPE_DRAW_STROKE:
                $this->_contents .= " S\n";
                break;
        }
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function drawImage(Pdf\Resource\Image\Image $image, $x1, $y1, $x2, $y2)
    {
        $this->_addProcSet('PDF');

        $imageName    = $this->_attachResource('XObject', $image);
        $imageNameObj = new Element\Name($imageName);

        $x1Obj     = new Element\Numeric($x1);
        $y1Obj     = new Element\Numeric($y1);
        $widthObj  = new Element\Numeric($x2 - $x1);
        $heightObj = new Element\Numeric($y2 - $y1);
    ...
    public function drawLine($x1, $y1, $x2, $y2)
    {
        ...
        $x1Obj = new Element\Numeric($x1);
        $y1Obj = new Element\Numeric($y1);
        $x2Obj = new Element\Numeric($x2);
        $y2Obj = new Element\Numeric($y2);
    ...
    public function drawPolygon($x, $y,
                                $fillType = Pdf\PagePdf\Page::SHAPE_DRAW_FILL_AND_STROKE,
                                $fillMethod = Pdf\PagePdf\Page::FILL_METHOD_NON_ZERO_WINDING)
    {
        $this->_addProcSet('PDF');
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function drawRectangle($x1, $y1, $x2, $y2, $fillType = Pdf\PagePdf\Page::SHAPE_DRAW_FILL_AND_STROKE)
    {
        $this->_addProcSet('PDF');
                                         ...
                                         $fillType = Pdf\PagePdf\Page::SHAPE_DRAW_FILL_AND_STROKE)
    {
        ...
        //draw top side
        $x1Obj = new Element\Numeric($topLeftX + $radius[0]);
        $y1Obj = new Element\Numeric($topLeftY);
        $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . " m\n";
        $x1Obj = new Element\Numeric($topRightX - $radius[1]);
        $y1Obj = new Element\Numeric($topRightY);
        $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . " l\n";

        //draw top right corner if needed
        if ($radius[1] != 0) {
            $x1Obj = new Element\Numeric($topRightX);
            $y1Obj = new Element\Numeric($topRightY);
            $x2Obj = new Element\Numeric($topRightX);
            $y2Obj = new Element\Numeric($topRightY);
            $x3Obj = new Element\Numeric($topRightX);
            $y3Obj = new Element\Numeric($topRightY - $radius[1]);
            $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . ' '
        ...
        //draw right side
        $x1Obj = new Element\Numeric($bottomRightX);
        $y1Obj = new Element\Numeric($bottomRightY + $radius[2]);
        $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . " l\n";

        //draw bottom right corner if needed
        if ($radius[2] != 0) {
            $x1Obj = new Element\Numeric($bottomRightX);
            $y1Obj = new Element\Numeric($bottomRightY);
            $x2Obj = new Element\Numeric($bottomRightX);
            $y2Obj = new Element\Numeric($bottomRightY);
            $x3Obj = new Element\Numeric($bottomRightX - $radius[2]);
            $y3Obj = new Element\Numeric($bottomRightY);
            $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . ' '
                              . $x2Obj->toString() . ' ' . $y2Obj->toString() . ' '
                              . $x3Obj->toString() . ' ' . $y3Obj->toString() . ' '
                              . " c\n";
        ...
        //draw bottom side
        $x1Obj = new Element\Numeric($bottomLeftX + $radius[3]);
        $y1Obj = new Element\Numeric($bottomLeftY);
        $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . " l\n";

        //draw bottom left corner if needed
        if ($radius[3] != 0) {
            $x1Obj = new Element\Numeric($bottomLeftX);
            $y1Obj = new Element\Numeric($bottomLeftY);
            $x2Obj = new Element\Numeric($bottomLeftX);
            $y2Obj = new Element\Numeric($bottomLeftY);
            $x3Obj = new Element\Numeric($bottomLeftX);
            $y3Obj = new Element\Numeric($bottomLeftY + $radius[3]);
            $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . ' '
                              . $x2Obj->toString() . ' ' . $y2Obj->toString() . ' '
                              . $x3Obj->toString() . ' ' . $y3Obj->toString() . ' '
                              . " c\n";
        ...
        //draw left side
        $x1Obj = new Element\Numeric($topLeftX);
        $y1Obj = new Element\Numeric($topLeftY - $radius[0]);
        $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . " l\n";
        ...
        if ($radius[0] != 0) {
            $x1Obj = new Element\Numeric($topLeftX);
            $y1Obj = new Element\Numeric($topLeftY);
            $x2Obj = new Element\Numeric($topLeftX);
            $y2Obj = new Element\Numeric($topLeftY);
            $x3Obj = new Element\Numeric($topLeftX + $radius[0]);
            $y3Obj = new Element\Numeric($topLeftY);
            $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() . ' '
                              . $x2Obj->toString() . ' ' . $y2Obj->toString() . ' '
                              . $x3Obj->toString() . ' ' . $y3Obj->toString() . ' '
                              . " c\n";
    ...
    public function drawText($text, $x, $y, $charEncoding = '')
    {
        if ($this->_font === null) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Pdf\Exception('Font has not been set');
        }

        $this->_addProcSet('Text');

        $textObj = new Pdf\Element\String\String($this->_font->encodeString($text, $charEncoding));
        $xObj    = new Element\Numeric($x);
        $yObj    = new Element\Numeric($y);
    ...
    {
        $cos  = new Element\Numeric(cos($angle));
        $sin  = new Element\Numeric(sin($angle));
        $mSin = new Element\Numeric(-$sin->value);

        $xObj = new Element\Numeric($x);
        $yObj = new Element\Numeric($y);

        $mXObj = new Element\Numeric(-$x);
        $mYObj = new Element\Numeric(-$y);
    ...
    {
        $xScaleObj = new Element\Numeric($xScale);
        $yScaleObj = new Element\Numeric($yScale);
    ...
    {
        $xShiftObj = new Element\Numeric($xShift);
        $yShiftObj = new Element\Numeric($yShift);
    ...
    {
        $tanXObj = new Element\Numeric(tan($xAngle));
        $tanYObj = new Element\Numeric(-tan($yAngle));
