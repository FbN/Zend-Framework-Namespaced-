 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Markup extends Annotation
{
    /**
     * Annotation subtypes
     */
    ...
    public static function create($x1, $y1, $x2, $y2, $text, $subType, $quadPoints)
    {
        ...
        $annotationDictionary->Subtype = new Element\Name($subType);
        ...
        $annotationDictionary->Contents = new Element\String\String($text);
        ...
        $points = new Element\Array();
        foreach ($quadPoints as $quadPoint) {
            $points->items[] = new Element\Numeric($quadPoint);
        }
        $annotationDictionary->QuadPoints = $points;

        return new Markup($annotationDictionary);
    }
}
