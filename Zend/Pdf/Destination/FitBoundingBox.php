...
class FitBoundingBox extends Explicit
{
    ...
    {
        $destinationArray = new \Zend\Pdf\Element\Array();

        if ($page instanceof \Zend\Pdf\Page) {
            $destinationArray->items[] = $page->getPageDictionary();
        } else if (is_integer($page)) {
            $destinationArray->items[] = new \Zend\Pdf\Element\Numeric($page);
        } else {
            require_once 'Zend/Pdf/Exception.php';
            throw new \Zend\Pdf\Exception('Page entry must be a Zend_Pdf_Page object or a page number.');
        }

        $destinationArray->items[] = new \Zend\Pdf\Element\Name('FitB');

        return new FitBoundingBox($destinationArray);
    }
}
