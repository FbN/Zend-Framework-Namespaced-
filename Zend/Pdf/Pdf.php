  ...
  /**** Class Constants ****/
    ...
    static public function getMemoryManager()
    {
        if (self::$_memoryManager === null) {
            ...
            self::$_memoryManager = \Zend\Memory\Memory::factory('none');
        }
     ...
     */
    static public function setMemoryManager(\Zend\Memory\Manager $memoryManager)
    {
    ...
    public function __construct($source = null, $revision = null, $load = false)
    {
        ...
        $this->_objFactory = ElementFactory\ElementFactory::createFactory(1);
        ...
        if ($source !== null) {
            ...
            $this->_pdfHeaderVersion = Pdf::PDF_VERSION;

            $trailerDictionary = new Element\Dictionary();
            ...
            $trailerDictionary->ID = new Element\Array();
            $trailerDictionary->ID->items[] = new Element\String\Binary($docIdLow);
            $trailerDictionary->ID->items[] = new Element\String\Binary($docIdHigh);

            $trailerDictionary->Size = new Element\Numeric(0);

            require_once 'Zend/Pdf/Trailer/Generator.php';
            $this->_trailer = new Trailer\Generator($trailerDictionary);

            /**
             * Document catalog indirect object.
             */
            $docCatalog = $this->_objFactory->newObject(new Element\Dictionary());
            $docCatalog->Type    = new Element\Name('Catalog');
            $docCatalog->Version = new Element\Name(Pdf::PDF_VERSION);
            $this->_trailer->Root = $docCatalog;

            /**
             * Pages container
             */
            $docPages = $this->_objFactory->newObject(new Element\Dictionary());
            $docPages->Type  = new Element\Name('Pages');
            $docPages->Kids  = new Element\Array();
            $docPages->Count = new Element\Numeric(0);
     ...
     */
    protected function _loadPages(Element\Reference\Reference $pages, $attributes = array())
    {
        if ($pages->getType() != Element\Element::TYPE_DICTIONARY) {
            require_once 'Zend/Pdf/Exception.php';
            ...
            if ($child->Type->value == 'Pages') {
                ...
                foreach (self::$_inheritableAttributes as $property) {
                    if ($child->$property === null && array_key_exists($property, $attributes)) {
                         ...
                         */
                        if ($attributes[$property] instanceof Element\Object\Object  ||
                            $attributes[$property] instanceof Element\Reference\Reference) {
     ...
     * @throws Zend_Pdf_Exception
     */
    protected function _loadNamedDestinations(Element\Reference\Reference $root, $pdfHeaderVersion)
    {
        ...
        if ($versionIs_1_2_plus) {
            ...
            if ($root->Names !== null  &&  $root->Names->Dests !== null) {
                ...
                foreach (new NameTree($root->Names->Dests) as $name => $destination) {
                    $this->_namedTargets[$name] = Target::load($destination);
                }
            }
        } else {
            ...
            if ($root->Dests !== null) {
                if ($root->Dests->getType() != Element\Element::TYPE_DICTIONARY) {
                    require_once 'Zend/Pdf/Exception.php';
                ...
                foreach ($root->Dests->getKeys() as $destKey) {
                    $this->_namedTargets[$destKey] = Target::load($root->Dests->$destKey);
                }
            }
        }
    }
     ...
     * @param Zend_Pdf_Element_Reference $root Document catalog entry
     */
    protected function _loadOutlines(Element\Reference\Reference $root)
    {
        if ($root->Outlines === null) {
            return;
        }

        if ($root->Outlines->getType() != Element\Element::TYPE_DICTIONARY) {
            require_once 'Zend/Pdf/Exception.php';
    ...
    protected function _dumpPages()
    {
        ...
        foreach ($this->_namedTargets as $name => $namedTarget) {
            ...
            } else if ($namedTarget instanceof Action\Action) {
        ...
        $iterator = new \RecursiveIteratorIterator(new RecursivelyIteratableObjectsContainer($this->outlines), \RecursiveIteratorIterator::SELF_FIRST);
        ...
        if ($openAction !== null) {
            if ($openAction instanceof Action\Action) {
                // OpenAction is an action
                if ($this->_cleanUpAction($openAction, false) === null) {
                    // Action is a GoTo action with an unresolved destination
                    $this->setOpenAction(null);
                }
            } else if ($openAction instanceof Destination\Destination) {
    ...
    protected function _dumpNamedDestinations()
    {
        ...
        foreach ($this->_namedTargets as $name => $destination) {
            $destArrayItems[] = new Element\String\String($name);
        ...
        $destArray = $this->_objFactory->newObject(new Element\Array($destArrayItems));

        $DestTree = $this->_objFactory->newObject(new Element\Dictionary());
        ...
        if ($root->Names === null) {
            $root->touch();
            $root->Names = $this->_objFactory->newObject(new Element\Dictionary());
        } else {
            $root->Names->touch();
        }
    ...
    protected function _dumpOutlines()
    {
        ...
        if ($root->Outlines === null) {
            if (count($this->outlines) == 0) {
                ...
                $root->Outlines = $this->_objFactory->newObject(new Element\Dictionary());
                $root->Outlines->Type = new Element\Name('Outlines');
        ...
        if ($openOutlinesCount != $this->_originalOpenOutlinesCount) {
            $root->Outlines->touch;
            $root->Outlines->Count = new Element\Numeric($openOutlinesCount);
        }
    }
    ...
    public function setMetadata($metadata)
    {
        ...
        $metadataObject->dictionary->Type    = new Element\Name('Metadata');
        $metadataObject->dictionary->Subtype = new Element\Name('XML');
    ...
    public function getOpenAction()
    {
        if ($this->_trailer->Root->OpenAction !== null) {
            require_once 'Zend/Pdf/Target.php';
            return Target::load($this->_trailer->Root->OpenAction);
        } else {
            return null;
        }
    }
     ...
     * @returns Zend_Pdf
     */
    public function setOpenAction(Target $openAction = null)
    {
        $root = $this->_trailer->Root;
        $root->touch();
    ...
    {
        if ($destination !== null  &&
            !$destination instanceof Action\GoTo  &&
     ...
     * @throws Zend_Pdf_Exception
     */
    public function resolveDestination(Destination\Destination $destination, $refreshPageCollectionHashes = true)
    {
        ...
        if ($destination instanceof Destination\Named) {
            ...
            if ($destination instanceof Action\Action) {
                if (!$destination instanceof Action\GoTo) {
                    return null;
                }
        ...
        if ($pageElement->getType() == Element\Element::TYPE_NUMERIC) {
     ...
     */
    protected function _cleanUpAction(Action\Action $action, $refreshPageCollectionHashes = true)
    {
        if ($this->_pageReferences === null  ||  $refreshPageCollectionHashes) {
            $this->_refreshPagesHash();
        }
        ...
        foreach ($iterator as $chainedAction) {
            if ($chainedAction instanceof Action\GoTo  &&
    ...
    public function extractFonts()
    {
        ...
        foreach ($this->pages as $page) {
            ...
            foreach ($fontResources->getKeys() as $fontResourceName) {
                ...
                if (! ($fontDictionary instanceof Element\Reference\Reference  ||
                       $fontDictionary instanceof Element\Object\Object) ) {
                    require_once 'Zend/Pdf/Exception.php';
    ...
    public function render($newSegmentOnly = false, $outputStream = null)
    {
        ...
        if ($this->properties != $this->_originalProperties) {
            $docInfo = $this->_objFactory->newObject(new Element\Dictionary());
            ...
            foreach ($this->properties as $key => $value) {
                switch ($key) {
                    case 'Trapped':
                        switch ($value) {
                            case true:
                                $docInfo->$key = new Element\Name('True');
                                break;

                            case false:
                                $docInfo->$key = new Element\Name('False');
                                break;

                            case null:
                                $docInfo->$key = new Element\Name('Unknown');
                                break;
                    ...
                    case 'ModDate':
                        $docInfo->$key = new Element\String\String((string)$value);
                        break;
                    ...
                    default:
                        ...
                        $docInfo->$key = Element\Element::phpToPdf($value);
                        break;
                }
            }
