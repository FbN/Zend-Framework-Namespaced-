 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Action extends Pdf\Target implements \RecursiveIterator, \Countable
{
    /**
     * Action dictionary
     *
    ...
    public function __construct(Element\Element $dictionary, \SplObjectStorage $processedActions)
    {
        ...
        if ($dictionary->Next !== null) {
            if ($dictionary->Next instanceof Element\Dictionary) {
                ...
                if (!$processedActions->contains($dictionary->Next)) {
                    ...
                    $this->next[] = Action::load($dictionary->Next, $processedActions);
                }
            } else if ($dictionary->Next instanceof Element\Array) {
                foreach ($dictionary->Next->items as $chainedActionDictionary) {
                    ...
                    if (!$processedActions->contains($chainedActionDictionary)) {
                        ...
                        $this->next[] = Action::load($chainedActionDictionary, $processedActions);
                    }
                }
            } else {
                require_once 'Zend/Pdf/Exception.php';
     ...
     * @throws Zend_Pdf_Exception
     */
    public static function load(Element\Element $dictionary, \SplObjectStorage $processedActions = null)
    {
        ...
        switch ($dictionary->S->value) {
            case 'GoTo':
                ...
                return new GoTo($dictionary, $processedActions);
                break;
     ...
     */
    public function dumpAction(ElementFactory\ElementFactoryInterface $factory, \SplObjectStorage $processedActions = null)
    {
        if ($processedActions === null) {
            $processedActions = new \SplObjectStorage();
        }
        ...
        if ($childListUpdated) {
            ...
            switch (count($this->next)) {
                ...
                default:
                    require_once 'Zend/Pdf/Element/Array.php';
                    $pdfChildArray = new Element\Array();
        ...
        if ($this->_actionDictionary instanceof Element\Dictionary) {
