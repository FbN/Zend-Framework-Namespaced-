 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Page extends Canvas\AbstractCanvas
{
  /**** Class Constants ****/
  ...
  /* Line Dash Types */
    ...
    {
        if (($param1 instanceof Element\Reference\Reference ||
             $param1 instanceof Element\Object\Object
            ) &&
            $param2 instanceof ElementFactory\ElementFactoryInterface &&
            $param3 === null
           ) {
            switch ($param1->getType()) {
                case Element\Element::TYPE_DICTIONARY:
                ...
                case Element\Element::TYPE_NULL:
                    $this->_objFactory = $param2;
            ...
            $this->_dictionary = $this->_objFactory->newObject(new Element\Dictionary());

            foreach ($param1->_dictionary->getKeys() as $key) {
                if ($key == 'Contents') {
                    // Clone Contents property

                    $this->_dictionary->Contents = new Element\Array();

                    if ($param1->_dictionary->Contents->getType() != Element\Element::TYPE_ARRAY) {
            ...
            return;
                   ...
                   ($param2 === null || $param2 instanceof ElementFactory\ElementFactoryInterface) &&
                   $param3 === null) {
            if ($param2 !== null) {
                $this->_objFactory = $param2;
            } else {
             ...
             */
                   ...
                   ($param3 === null || $param3 instanceof ElementFactory\ElementFactoryInterface)) {
            if ($param3 !== null) {
        ...
        $this->_dictionary = $this->_objFactory->newObject(new Element\Dictionary());
        $this->_dictionary->Type         = new Element\Name('Page');
        require_once 'Zend/Pdf.php';
        $this->_dictionary->LastModified = new Element\String\String(Pdf::pdfDate());
        $this->_dictionary->Resources    = new Element\Dictionary();
        $this->_dictionary->MediaBox     = new Element\Array();
        $this->_dictionary->MediaBox->items[] = new Element\Numeric(0);
        $this->_dictionary->MediaBox->items[] = new Element\Numeric(0);
        $this->_dictionary->MediaBox->items[] = new Element\Numeric($pageWidth);
        $this->_dictionary->MediaBox->items[] = new Element\Numeric($pageHeight);
        $this->_dictionary->Contents     = new Element\Array();
    }
     ...
     * @return string
     */
    protected function _attachResource($type, Resource\Resource $resource)
    {
        ...
        if ($this->_dictionary->Resources->$type === null) {
            ...
            $this->_dictionary->Resources->$type = new Element\Dictionary();
        } else {
    ...
    protected function _addProcSet($procSetName)
    {
        ...
        if ($this->_dictionary->Resources->ProcSet === null) {
            $this->_dictionary->Resources->touch();
            $this->_dictionary->Resources->ProcSet = new Element\Array();
        } else {
        ...
        $this->_dictionary->Resources->ProcSet->items[] = new Element\Name($procSetName);
    }
        ...
        foreach ($resDictionary->getKeys() as $resType) {
            ...
            if ($resType == 'ProcSet') {
                ...
                foreach ($resMap->getKeys() as $resId) {
                    $resources[$resType][$resId] =new Resource\Unified($resMap->$resId);
                }
            }
        }
    ...
    public function __clone()
    {
        ...
        foreach ($this->_dictionary->getKeys() as $key) {
            $dictionary->$key = $this->_dictionary->$key->makeClone($factory->getFactory(),
                                                                        $processed,
                                                                        Element\Element::CLONE_MODE_SKIP_PAGES);
        }
    ...
    public function flush()
    {
        ...
        if ($this->_dictionary->Contents->getType() != Element\Element::TYPE_ARRAY) {
            /**
        ...
        if ((!$this->_safeGS)  &&  (count($this->_dictionary->Contents->items) != 0)) {
            ...
            $newContentsArray = new Element\Array();
     ...
     * @throws Zend_Pdf_Exception
     */
    public function render(ElementFactory\ElementFactoryInterface $objFactory)
    {
        $this->flush();
     ...
     * @return Zend_Pdf_Page
     */
    public function attachAnnotation(Annotation\Annotation $annotation)
    {
        $annotationDictionary = $annotation->getResource();
        if (!$annotationDictionary instanceof Element\Object\Object  &&
            !$annotationDictionary instanceof Element\Reference\Reference) {
            $annotationDictionary = $this->_objFactory->newObject($annotationDictionary);
        }

        if ($this->_dictionary->Annots === null) {
            $this->_dictionary->touch();
            $this->_dictionary->Annots = new Element\Array();
        } else {
