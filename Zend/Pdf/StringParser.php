...
//        }
    ...
    public function readElement($nextLexeme = null)
    {
        ...
        switch ($nextLexeme) {
            ...
            case '/':
                return ($this->_elements[] = new Element\Name(
                                                    Element\Name::unescape( $this->readLexeme() )
                                                                      ));
            ...
            case '}':
                ...
                throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X.',
                                                $this->offset));

            default:
                if (strcasecmp($nextLexeme, 'true') == 0) {
                    return ($this->_elements[] = new Element\Boolean(true));
                } else if (strcasecmp($nextLexeme, 'false') == 0) {
                    return ($this->_elements[] = new Element\Boolean(false));
                } else if (strcasecmp($nextLexeme, 'null') == 0) {
                    return ($this->_elements[] = new Element\Null());
                }
    ...
    private function _readString()
    {
        ...
        if ($openedBrackets != 0) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Unexpected end of file while string reading. Offset - 0x%X. \')\' expected.', $start));
        }

        return new Element\String\String(Element\String\String::unescape( substr($this->data,
                                                                                     $start,
                                                                                     $this->offset - $start - 1) ));
    }
    ...
    private function _readBinaryString()
    {
        ...
        if ($this->offset >= strlen($this->data) - 1) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Unexpected end of file while reading binary string. Offset - 0x%X. \'>\' expected.', $start));
        }

        if ($this->data[$this->offset++] != '>') {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Unexpected character while binary string reading. Offset - 0x%X.', $this->offset));
        }

        return new Element\String\Binary(
                       Element\String\Binary::unescape( substr($this->data,
                                                                        $start,
                                                                        $this->offset - $start - 1) ));
    }
    ...
    private function _readArray()
    {
        ...
        while ( strlen($nextLexeme = $this->readLexeme()) != 0 ) {
            if ($nextLexeme != ']') {
                ...
                return new Element\Array($elements);
            }
        }

        require_once 'Zend/Pdf/Exception.php';
        throw new Exception(sprintf('PDF file syntax error. Unexpected end of file while array reading. Offset - 0x%X. \']\' expected.', $this->offset));
    }
    ...
    private function _readDictionary()
    {
        ...
        while ( strlen($nextLexeme = $this->readLexeme()) != 0 ) {
            if ($nextLexeme != '>>') {
                ...
                if (!$name instanceof Element\Name) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Exception(sprintf('PDF file syntax error. Name object expected while dictionary reading. Offset - 0x%X.', $nameStart));
                }
        ...
        require_once 'Zend/Pdf/Exception.php';
        throw new Exception(sprintf('PDF file syntax error. Unexpected end of file while dictionary reading. Offset - 0x%X. \'>>\' expected.', $this->offset));
    }
    ...
    private function _readReference($nextLexeme = null)
    {
        ...
        $ref = new Element\Reference\Reference((int)$objNum, (int)$genNum, $this->_context, $this->_objFactory->resolve());
    ...
    private function _readNumeric($nextLexeme = null)
    {
        ...
        return new Element\Numeric($nextLexeme);
    }
     ...
     */
    public function getObject($offset, Element\Reference\Context $context)
    {
        if ($offset === null ) {
        ...
        if (!ctype_digit($objNum)) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. Object number expected.', $this->offset - strlen($objNum)));
        }

        $genNum = $this->readLexeme();
        if (!ctype_digit($genNum)) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. Object generation number expected.', $this->offset - strlen($genNum)));
        }

        $objKeyword = $this->readLexeme();
        if ($objKeyword != 'obj') {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. \'obj\' keyword expected.', $this->offset - strlen($objKeyword)));
        }
        ...
        if( $nextLexeme == 'endobj' ) {
             ...
             */
            $obj = new Element\Object\Object($objValue, (int)$objNum, (int)$genNum, $this->_objFactory->resolve());
        ...
        if ($nextLexeme != 'stream') {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. \'endobj\' or \'stream\' keywords expected.', $this->offset - strlen($nextLexeme)));
        }

        if (!$objValue instanceof Element\Dictionary) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. Stream extent must be preceded by stream dictionary.', $this->offset - strlen($nextLexeme)));
        }
        ...
        if ($this->data[$this->offset] == "\r" &&
            ...
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. \'stream\' must be followed by either cr-lf sequence or lf character only.', $this->offset - strlen($nextLexeme)));
        }
        ...
        if ($nextLexeme != 'endstream') {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. \'endstream\' keyword expected.', $this->offset - strlen($nextLexeme)));
        }

        $nextLexeme = $this->readLexeme();
        if ($nextLexeme != 'endobj') {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception(sprintf('PDF file syntax error. Offset - 0x%X. \'endobj\' keyword expected.', $this->offset - strlen($nextLexeme)));
        }

        $obj = new Element\Object\Stream(substr($this->data,
                                                         $dataOffset,
                                                         $streamLength),
                                                  (int)$objNum,
                                                  (int)$genNum,
     ...
     * @param Zend_Pdf_Element_Reference_Context $context
     */
    public function setContext(Element\Reference\Context $context)
    {
        $this->_context = $context;
    }
     ...
     */
    public function __construct($source, ElementFactory\ElementFactoryInterface $factory)
    {
        $this->data         = $source;
