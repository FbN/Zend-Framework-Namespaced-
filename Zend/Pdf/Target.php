...
abstract class Target
{
    ...
    public static function load(Element\Element $resource) {
        ...
        if ($resource->getType() == Element\Element::TYPE_ARRAY  ||
            $resource->getType() == Element\Element::TYPE_NAME   ||
            $resource->getType() == Element\Element::TYPE_STRING) {
            // Resource is an array, just treat it as an explicit destination array
            require_once 'Zend/Pdf/Destination.php';
            return Destination\Destination::load($resource);
        } else {
            require_once 'Zend/Pdf/Exception.php';
            throw new Exception( 'Wrong resource type.' );
        }
