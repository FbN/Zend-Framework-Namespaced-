...
class Type0 extends Font
{
     ...
     *
     */
    public function __construct(CidFont\CidFont $descendantFont)
    {
        parent::__construct();

        $this->_objectFactory->attach($descendantFont->getFactory());

        $this->_fontType       = \Zend\Pdf\Font::TYPE_TYPE_0;
        ...
        $this->_resource->Subtype         = new Element\Name('Type0');
        $this->_resource->BaseFont        = new Element\Name($descendantFont->getResource()->BaseFont->value);
        $this->_resource->DescendantFonts = new Element\Array(array( $descendantFont->getResource() ));
        $this->_resource->Encoding        = new Element\Name('Identity-H');
