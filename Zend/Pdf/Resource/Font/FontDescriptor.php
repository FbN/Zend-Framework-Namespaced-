...
class FontDescriptor
{
    ...
    public function __construct()
    {
        ...
        throw new Pdf\Exception('Zend_Pdf_Resource_Font_FontDescriptor is not intended to be instantiated');
    }
     ...
     * @throws Zend_Pdf_Exception
     */
    static public function factory(Font $font, Pdf\FileParser\Font\OpenType\OpenType $fontParser, $embeddingOptions)
    {
         ...
         */
        $fontDescriptor = new Pdf\Element\Dictionary();

        $fontDescriptor->Type     = new Element\Name('FontDescriptor');
        $fontDescriptor->FontName = new Element\Name($font->getResource()->BaseFont->value);
        ...
        $fontDescriptor->Flags = new Element\Numeric($flags);

        $fontBBox = array(new Element\Numeric($font->toEmSpace($fontParser->xMin)),
                          new Element\Numeric($font->toEmSpace($fontParser->yMin)),
                          new Element\Numeric($font->toEmSpace($fontParser->xMax)),
                          new Element\Numeric($font->toEmSpace($fontParser->yMax)));
        $fontDescriptor->FontBBox     = new Pdf\Element\Array($fontBBox);

        $fontDescriptor->ItalicAngle  = new Element\Numeric($fontParser->italicAngle);

        $fontDescriptor->Ascent       = new Element\Numeric($font->toEmSpace($fontParser->ascent));
        $fontDescriptor->Descent      = new Element\Numeric($font->toEmSpace($fontParser->descent));

        $fontDescriptor->CapHeight    = new Element\Numeric($fontParser->capitalHeight);
        /**
         ...
         */
        $fontDescriptor->StemV        = new Element\Numeric(0);

        $fontDescriptor->MissingWidth = new Element\Numeric($fontParser->glyphWidths[0]);
        ...
        if (!($embeddingOptions & Pdf\Font::EMBED_DONT_EMBED)) {
            ...
            if (! $fontParser->isEmbeddable) {
                ...
                $fontFileObject->dictionary->Length1 = new Element\Numeric(strlen($fontFile));
                if (!($embeddingOptions & Pdf\Font::EMBED_DONT_COMPRESS)) {
                    /* Compress the font file using Flate. This generally cuts file
                     * sizes by about half!
                     */
                    $fontFileObject->dictionary->Filter = new Element\Name('FlateDecode');
                }
                if ($fontParser instanceof Pdf\FileParser\Font\OpenType\Type1 /* not implemented now */) {
                    $fontDescriptor->FontFile  = $fontFileObject;
                } else if ($fontParser instanceof Pdf\FileParser\Font\OpenType\TrueType) {
