 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class CidFont extends Pdf\Resource\Font\Font
{
    /**
     ...
     */
    public function __construct(Pdf\FileParser\Font\OpenType\OpenType $fontParser)
    {
        parent::__construct();
        ...
        $this->_resource->BaseFont = new Pdf\Element\Name($baseFont);
        ...
        $this->_resource->DW = new Element\Numeric($this->toEmSpace($defaultWidth));
            ...
            foreach ($widthsSequence as $width) {
                if ($lastWidth != $width) {
                    ...
                    if ($widthsInSequence != 0) {
                        ...
                        $pdfCharsWidths[] = new Element\Numeric($startCode);                         // First character code
                        $pdfCharsWidths[] = new Element\Numeric($startCode + $widthsInSequence - 1); // Last character code
                        $pdfCharsWidths[] = new Element\Numeric($this->toEmSpace($lastWidth));       // Width
                    ...
                    // Collect new width
                    $pdfWidths[] = new Element\Numeric($this->toEmSpace($width));
                    ...
                    if (count($pdfWidths) != 0) {
                        ...
                        if (count($pdfWidths) != 0) {
                            ...
                            $pdfCharsWidths[] = new Element\Numeric($startCode); // First character code
                            $pdfCharsWidths[] = new Element\Array($pdfWidths);   // Widths array
         ...
         */
        $widthsArrayElement = new Element\Array($pdfCharsWidths);
        $widthsObject = $this->_objectFactory->newObject($widthsArrayElement);
        $this->_resource->W = $widthsObject;


        /* CIDSystemInfo dictionary */
        $cidSystemInfo = new Pdf\Element\Dictionary();
        $cidSystemInfo->Registry   = new String\String('Adobe');
        $cidSystemInfo->Ordering   = new String\String('UCS');
        $cidSystemInfo->Supplement = new Element\Numeric(0);
