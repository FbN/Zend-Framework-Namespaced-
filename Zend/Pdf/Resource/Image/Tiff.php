...
class Tiff extends Image
{
    ...
    public function __construct($imageFileName)
    {
        ...
        $imageDictionary->Width            = new Element\Numeric($this->_width);
        if($this->_whiteIsZero === true) {
            $imageDictionary->Decode       = new Element\Array(array(new Element\Numeric(1), new Element\Numeric(0)));
        }
        $imageDictionary->Height           = new Element\Numeric($this->_height);
        $imageDictionary->ColorSpace       = new Element\Name($this->_colorSpace);
        $imageDictionary->BitsPerComponent = new Element\Numeric($this->_bitsPerSample);
        if(isset($this->_filter) && $this->_filter != 'None') {
            $imageDictionary->Filter = new Element\Name($this->_filter);
        }
