...
class Jpeg extends Image
{
    ...
    public function __construct($imageFileName)
    {
        ...
        $imageDictionary->Width            = new Element\Numeric($imageInfo[0]);
        $imageDictionary->Height           = new Element\Numeric($imageInfo[1]);
        $imageDictionary->ColorSpace       = new Element\Name($colorSpace);
        $imageDictionary->BitsPerComponent = new Element\Numeric($imageInfo['bits']);
        if ($imageInfo[2] == IMAGETYPE_JPEG) {
            $imageDictionary->Filter       = new Element\Name('DCTDecode');
        } else if ($imageInfo[2] == IMAGETYPE_JPEG2000){
            $imageDictionary->Filter       = new Element\Name('JPXDecode');
        }
        ...
        while ($byteCount > 0 && !feof($imageFile)) {
            ...
            if ($nextBlock === false) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Pdf\Exception( "Error occured while '$imageFileName' file reading." );
            }
