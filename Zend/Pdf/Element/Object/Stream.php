 ...
 * @namespace
 */
namespace Zend\Pdf\Element\Object;
use Zend\Pdf\ElementFactory;
use Zend\Pdf\Element;
use Zend\Pdf;
use Zend\Pdf\Filter;
use Zend\Pdf\Filter;
use Zend\Pdf\Filter\Compression;
use Zend\Pdf\Filter\Compression;
use Zend\Pdf\Filter;
...
class Stream extends Object
{
     ...
     */
    public function __construct($val, $objNum, $genNum, ElementFactory\ElementFactory $factory, $dictionary = null)
    {
        parent::__construct(new Pdf\Element\Stream($val), $objNum, $genNum, $factory);

        if ($dictionary === null) {
            $this->_dictionary    = new Pdf\Element\Dictionary();
            $this->_dictionary->Length = new Pdf\Element\Numeric(strlen( $val ));
    ...
    private function _extractDictionaryData()
    {
        ...
        if ($this->_dictionary->Filter === null) {
            // Do nothing.
        } else if ($this->_dictionary->Filter->getType() == Element\Element::TYPE_ARRAY) {
        ...
        } else if ($this->_dictionary->Filter->getType() != Element\Element::TYPE_NULL) {
        ...
        if ($this->_dictionary->FFilter === null) {
            // Do nothing.
        } else if ($this->_dictionary->FFilter->getType() == Element\Element::TYPE_ARRAY) {
    ...
    private function _encodeStream()
    {
        ...
        foreach ($filters as $id => $filterName ) {
            ...
            switch ($filterName) {
                case 'ASCIIHexDecode':
                    require_once 'Zend/Pdf/Filter/AsciiHex.php';
                    $valueRef = Filter\AsciiHex::encode($valueRef);
                    break;

                case 'ASCII85Decode':
                    require_once 'Zend/Pdf/Filter/Ascii85.php';
                    $valueRef = Filter\Ascii85::encode($valueRef);
                    break;

                case 'FlateDecode':
                    require_once 'Zend/Pdf/Filter/Compression/Flate.php';
                    $valueRef = Compression\Flate::encode($valueRef,
                                                                          $this->_initialDictionaryData['DecodeParms'][$id]);
                    break;

                case 'LZWDecode':
                    require_once 'Zend/Pdf/Filter/Compression/Lzw.php';
                    $valueRef = Compression\Lzw::encode($valueRef,
                                                                        $this->_initialDictionaryData['DecodeParms'][$id]);
                    break;

                 case 'RunLengthDecode':
                    require_once 'Zend/Pdf/Filter/RunLength.php';
                    $valueRef = Filter\RunLength::encode($valueRef);
                    break;
     ...
     */
    public function makeClone(ElementFactory\ElementFactory $factory, array &$processed, $mode)
    {
     ...
     * @return string
     */
    public function dump(ElementFactory\ElementFactory $factory)
    {
