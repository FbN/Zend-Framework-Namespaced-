...
abstract class FileParser
{
  ...
  /* Object Lifecycle */
     ...
     * @throws Zend_Pdf_Exception
     */
    public function __construct(ataSource\FileParserDataSource $dataSource)
    {
  ...
  /* Parser Methods */
     ...
     * @throws Zend_Pdf_Exception
     */
    public function readInt($size, $byteOrder = FileParserFileParser::BYTE_ORDER_BIG_ENDIAN)
    {
        if (($size < 1) || ($size > 4)) {
            require_once 'Zend/Pdf/Exception.php';
         ...
         */
        if ($byteOrder == FileParser::BYTE_ORDER_BIG_ENDIAN) {
            $number = ord($bytes[0]);
            if (($number & 0x80) == 0x80) {
        ...
        } else if ($byteOrder == FileParser::BYTE_ORDER_LITTLE_ENDIAN) {
     ...
     * @throws Zend_Pdf_Exception
     */
    public function readUInt($size, $byteOrder = FileParserFileParser::BYTE_ORDER_BIG_ENDIAN)
    {
        if (($size < 1) || ($size > 4)) {
            require_once 'Zend/Pdf/Exception.php';
    ...
    public function readFixed($mantissaBits, $fractionBits,
                              $byteOrder = FileParserFileParser::BYTE_ORDER_BIG_ENDIAN)
    {
