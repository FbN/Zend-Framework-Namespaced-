    ...
    implements Feed\Writer\Renderer\RendererInterface
{
    ...
    protected function _setEnclosure(\DOMDocument $dom, \DOMElement $root)
    {
        ...
        if (!isset($data['type'])) {
            require_once 'Zend/Feed/Exception.php';
            $exception = new Feed\Exception('Enclosure "type" is not set');
            if (!$this->_ignoreExceptions) {
                throw $exception;
            } else {
                $this->_exceptions[] = $exception;
        ...
        if (!isset($data['length'])) {
            require_once 'Zend/Feed/Exception.php';
            $exception = new Feed\Exception('Enclosure "length" is not set');
            if (!$this->_ignoreExceptions) {
                throw $exception;
            } else {
                $this->_exceptions[] = $exception;
        ...
        if (isset($data['length']) && (int) $data['length'] <= 0) {
            require_once 'Zend/Feed/Exception.php';
            $exception = new Feed\Exception('Enclosure "length" must be an integer'
    ...
    protected function _setId(\DOMDocument $dom, \DOMElement $root)
    {
        ...
        $id->appendChild($text);
        if (!\Zend\Uri\Uri::check($this->getDataContainer()->getId())) {
