...
class FeedAbstract
{
    ...
    public function addAuthor($name, $email = null, $uri = null)
    {
        ...
        if (is_array($name)) {
            ...
            if (isset($name['uri'])) {
                if (empty($name['uri']) || !is_string($name['uri']) || !Uri\Uri::check($name['uri'])) {
                    require_once 'Zend/Feed/Exception.php';
            ...
            if (isset($uri)) {
                if (empty($uri) || !is_string($uri) || !Uri\Uri::check($uri)) {
                    require_once 'Zend/Feed/Exception.php';
    ...
    public function setGenerator($name, $version = null, $uri = null)
    {
        if (is_array($name)) {
            ...
            if (isset($data['uri'])) {
                if (empty($data['uri']) || !is_string($data['uri']) || !Uri\Uri::check($data['uri'])) {
                    require_once 'Zend/Feed/Exception.php';
    ...
    {
        if ((empty($id) || !is_string($id) || !Uri\Uri::check($id)) &&
    ...
    public function setImage(array $data)
    {
        ...
        || !Uri\Uri::check($data['uri'])) {
            require_once 'Zend/Feed/Exception.php';
            throw new Feed\Exception('Invalid parameter: parameter \'uri\''
    ...
    {
        if (empty($link) || !is_string($link) || !Uri\Uri::check($link)) {
            require_once 'Zend/Feed/Exception.php';
            throw new Feed\Exception('Invalid parameter: parameter must be a non-empty string and valid URI/IRI');
        }
    ...
    {
        if (empty($url) || !is_string($url) || !Uri\Uri::check($url)) {
            require_once 'Zend/Feed/Exception.php';
