 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Pdf extends RendererAbstract
{
    /**
     * PDF resource
     * @var Zend_Pdf
    ...
    public function setResource($pdf, $page = 0)
    {
        ...
        if (!count($this->_resource->pages)) {
            $this->_page = 0;
            $this->_resource->pages[] = new Pdf\Page(
                Pdf\Page::SIZE_A4
            );
        }
        return $this;
    ...
    protected function _drawText(
        ...
        $page->setLineColor($color);
        $page->setFillColor($color);
        $page->setFont(Pdf\Font::fontWithPath($font), $size * $this->_moduleSize * 1.2);

        $width = $this->widthForStringUsingFontSize(
            $text,
            Pdf\Font::fontWithPath($font),
