...
class LocalizedToNormalized implements FilterInterface
{
    ...
    {
        if (Locale\Format::isNumber($value, $this->_options)) {
            return Locale\Format::getNumber($value, $this->_options);
        } else if (($this->_options['date_format'] === null) && (strpos($value, ':') !== false)) {
            // Special case, no date format specified, detect time input
            return Locale\Format::getTime($value, $this->_options);
        } else if (Locale\Format::checkDateFormat($value, $this->_options)) {
            // Detect date or time input
            return Locale\Format::getDate($value, $this->_options);
        }
