 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Inflector implements FilterInterface
{
    /**
    ...
    public function getPluginLoader()
    {
        if (!$this->_pluginLoader instanceof PluginLoader\PluginLoaderInterface) {
            $this->_pluginLoader = new PluginLoader\PluginLoader(array('Zend_Filter_' => 'Zend/Filter/'), __CLASS__);
        }
    ...
    protected function _getRule($rule)
    {
        ...
        if (!$ruleObject \instanceof FilterInterface) {
            require_once 'Zend/Filter/Exception.php';
            throw new Exception('No class named ' . $rule . ' implementing Zend_Filter_Interface could be found');
        }
