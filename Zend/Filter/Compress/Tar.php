 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Tar extends CompressAbstract
{
    /**
     * Compression Options
     * array(
    ...
    public function __construct($options = null)
    {
        if (!class_exists('Archive_Tar')) {
            ...
            try {
                \Zend\Loader\Loader::loadClass('Archive_Tar');
            } catch (\Zend\Exception $e) {
                require_once 'Zend/Filter/Exception.php';
                throw new Filter\Exception('This filter needs PEARs Archive_Tar', 0, $e);
            }
        }
    ...
    public function setTarget($target)
    {
        if (!file_exists(dirname($target))) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception("The directory '$target' does not exist");
        }
    ...
    public function setMode($mode)
    {
        ...
        if (($mode != 'Bz2') && ($mode != 'Gz')) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception("The mode '$mode' is unknown");
        }

        if (($mode == 'Bz2') && (!extension_loaded('bz2'))) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('This mode needs the bz2 extension');
        }

        if (($mode == 'Gz') && (!extension_loaded('zlib'))) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('This mode needs the zlib extension');
        }
    }
    ...
    {
        $archive = new \Archive\Tar($this->getArchive(), $this->getMode());
        if (!file_exists($content)) {
            ...
            if ($result === false) {
                require_once 'Zend/Filter/Exception.php';
                throw new Filter\Exception('Error creating the temporary file');
            }
        ...
        if ($result === false) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Error creating the Tar archive');
        }
    ...
    public function decompress($content)
    {
        ...
        if (file_exists($content)) {
            ...
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Tar Archive not found');
        }

        $archive = new \Archive\Tar($archive, $this->getMode());
        ...
        if ($result === false) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Error while extracting the Tar archive');
        }
