 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Gz extends CompressAbstract
{
    /**
     * Compression Options
     * array(
    ...
    public function __construct($options = null)
    {
        if (!extension_loaded('zlib')) {
            ...
            throw new Filter\Exception('This filter needs the zlib extension');
        }
    ...
    public function setLevel($level)
    {
        if (($level < 0) || ($level > 9)) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Level must be between 0 and 9');
        }
    ...
    public function setMode($mode)
    {
        if (($mode != 'compress') && ($mode != 'deflate')) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Given compression mode not supported');
        }
    ...
    public function compress($content)
    {
        ...
        if (!empty($archive)) {
            ...
            if (!$file) {
                require_once 'Zend/Filter/Exception.php';
                throw new Filter\Exception("Error opening the archive '" . $this->_options['archive'] . "'");
            }
        ...
        if (!$compressed) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Error during compression');
        }
    ...
    public function decompress($content)
    {
        ...
        if (file_exists($archive)) {
            ...
            if (!$handler) {
                require_once 'Zend/Filter/Exception.php';
                throw new Filter\Exception("Error opening the archive '" . $archive . "'");
            }
