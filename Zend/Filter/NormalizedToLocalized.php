...
class NormalizedToLocalized implements FilterInterface
{
    ...
    public function filter($value)
    {
        if (is_array($value)) {
            ...
            $date = new \Zend\Date\Date($value, $this->_options['locale']);
            return $date->toString($this->_options['date_format']);
        } else if ($this->_options['precision'] === 0) {
            return Locale\Format::toInteger($value, $this->_options);
        } else if ($this->_options['precision'] === null) {
            return Locale\Format::toFloat($value, $this->_options);
        }

        return Locale\Format::toNumber($value, $this->_options);
    }
}
