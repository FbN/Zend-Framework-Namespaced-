...
class Mcrypt implements EncryptInterface
{
    ...
    public function __construct($options)
    {
        if (!extension_loaded('mcrypt')) {
            ...
            throw new Filter\Exception('This filter needs the mcrypt extension');
        }
    ...
    public function setEncryption($options)
    {
        ...
        if (!in_array($options['algorithm'], $algorithms)) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception("The algorithm '{$options['algorithm']}' is not supported");
        }

        $modes = mcrypt_list_modes($options['mode_directory']);
        if (!in_array($options['mode'], $modes)) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception("The mode '{$options['mode']}' is not supported");
        }

        if (!mcrypt_module_self_test($options['algorithm'], $options['algorithm_directory'])) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('The given algorithm can not be used due an internal mcrypt problem');
        }
    ...
    public function setVector($vector = null)
    {
        ...
        if (empty($vector)) {
            ...
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('The given vector has a wrong size for the set algorithm');
        }
    ...
    public function encrypt($value)
    {
        ...
        if (!empty($this->_compression)) {
            ...
            $compress = new Filter\Compress\Compress($this->_compression);
    ...
    public function decrypt($value)
    {
        ...
        if (!empty($this->_compression)) {
            ...
            $decompress = new Filter\Decompress($this->_compression);
    ...
    protected function _openCipher()
    {
        ...
        if ($cipher === false) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Mcrypt can not be opened with your settings');
        }
    ...
    protected function _initCipher($cipher)
    {
        ...
        if (empty($keysizes) || ($this->_encryption['salt'] == true)) {
            ...
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('The given key has a wrong size for the set algorithm');
        }

        $result = mcrypt_generic_init($cipher, $key, $this->_encryption['vector']);
        if ($result < 0) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Mcrypt could not be initialize with the given setting');
        }
