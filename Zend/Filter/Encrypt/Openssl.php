...
class Openssl implements EncryptInterface
{
    ...
    public function __construct($options = array())
    {
        if (!extension_loaded('openssl')) {
            ...
            throw new Filter\Exception('This filter needs the openssl extension');
        }
        ...
        foreach ($keys as $type => $key) {
            ...
            switch ($type) {
                case 'public':
                    ...
                    if ($test === false) {
                        require_once 'Zend/Filter/Exception.php';
                        throw new Filter\Exception("Public key '{$cert}' not valid");
                    }
                ...
                case 'private':
                    ...
                    if ($test === false) {
                        require_once 'Zend/Filter/Exception.php';
                        throw new Filter\Exception("Private key '{$cert}' not valid");
                    }
    ...
    public function encrypt($value)
    {
        ...
        if (count($this->_keys['public']) == 0) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Openssl can not encrypt without public keys');
        }
        ...
        if ($crypt === false) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Openssl was not able to encrypt your content with the given options');
        }
    ...
    public function decrypt($value)
    {
        ...
        if (count($this->_keys['private']) !== 1) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Please give a private key for decryption with Openssl');
        }

        if (!$this->_package && empty($envelope)) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Please give a envelope key for decryption with Openssl');
        }
        ...
        if ($crypt === false) {
            require_once 'Zend/Filter/Exception.php';
            throw new Filter\Exception('Openssl was not able to decrypt you content with the given options');
        }
