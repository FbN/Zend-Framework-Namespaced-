 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Docblock extends \Zend\CodeGenerator\Php\AbstractPhp
{
    /**
     * @var string
     */
    ...
    public static function fromReflection(\Zend\Reflection\Docblock\Docblock $reflectionDocblock)
    {
        ...
        foreach ($reflectionDocblock->getTags() as $tag) {
            $docblock->setTag(Tag\Tag::fromReflection($tag));
        }
    ...
    public function setTag($tag)
    {
        if (is_array($tag)) {
            $tag = new Tag\Tag($tag);
        } elseif (!$tag instanceof Tag\Tag) {
            require_once 'Zend/CodeGenerator/Php/Exception.php';
            throw new \Zend\CodeGenerator\Php\Exception(
