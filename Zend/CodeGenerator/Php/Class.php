 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Class extends AbstractPhp
{
    ...
    public static function fromReflection(\Zend\Reflection\Class $reflectionClass)
    {
        ...
        if ($reflectionClass->getDocComment() != '') {
            $class->setDocblock(Docblock\Docblock::fromReflection($reflectionClass->getDocblock()));
        }
        ...
        foreach ($reflectionClass->getProperties() as $reflectionProperty) {
            if ($reflectionProperty->getDeclaringClass()->getName() == $class->getName()) {
                $properties[] = Property\Property::fromReflection($reflectionProperty);
            }
        }
        ...
        foreach ($reflectionClass->getMethods() as $reflectionMethod) {
            if ($reflectionMethod->getDeclaringClass()->getName() == $class->getName()) {
                $methods[] = Method::fromReflection($reflectionMethod);
            }
        }
    ...
    public function setDocblock($docblock)
    {
        ...
        if (is_array($docblock)) {
            $docblock = new Docblock\Docblock($docblock);
        } elseif (!$docblock instanceof Docblock\Docblock) {
    ...
    public function setProperty($property)
    {
        if (is_array($property)) {
            $property = new Property\Property($property);
            $propertyName = $property->getName();
        } elseif ($property instanceof Property\Property) {
            $propertyName = $property->getName();
        } else {
            require_once 'Zend/CodeGenerator/Php/Exception.php';
    ...
    {
        $this->_properties = new Member\Container(Member\Container::TYPE_PROPERTY);
        $this->_methods = new Member\Container(Member\Container::TYPE_METHOD);
    }
