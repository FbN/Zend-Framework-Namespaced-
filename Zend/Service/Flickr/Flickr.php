...
class Flickr
{
    ...
    public function groupPoolGetPhotos($query, array $options = array())
    {
        ...
        if (empty($query) || !is_string($query)) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new Service\Exception('You must supply a group id');
        }
    ...
    public function getIdByUsername($username)
    {
        ...
        if (empty($username)) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new Service\Exception('You must supply a username');
        }
    ...
    public function getIdByEmail($email)
    {
        ...
        if (empty($email)) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new Service\Exception('You must supply an e-mail address');
        }
    ...
    public function getImageDetails($id)
    {
        ...
        if (empty($id)) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new Service\Exception('You must supply a photo ID');
        }
    ...
    public function getRestClient()
    {
        if (null === $this->_restClient) {
            ...
            $this->_restClient = new \Zend\Rest\Client\Client(self::URI_BASE);
        }
    ...
    protected function _validateUserSearch(array $options)
    {
        ...
        $between = new Validate\Between(1, 500, true);
        if (!$between->isValid($options['per_page'])) {
            /**
             * @see Zend_Service_Exception
             */
            require_once 'Zend/Service/Exception.php';
            throw new Service\Exception($options['per_page'] . ' is not valid for the "per_page" option');
        }
        ...
        $int = new Validate\Int();
        if (!$int->isValid($options['page'])) {
            /**
             * @see Zend_Service_Exception
             */
            require_once 'Zend/Service/Exception.php';
            throw new Service\Exception($options['page'] . ' is not valid for the "page" option');
        }
    ...
    */
    ...
    protected static function _checkErrors(\DOMDocument $dom)
    {
        if ($dom->documentElement->getAttribute('stat') === 'fail') {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new Service\Exception('Search failed due to error: ' . $err->getAttribute('msg')
    ...
    protected function _compareOptions(array $options, array $validOptions)
    {
        ...
        if ($difference) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new Service\Exception('The following parameters are invalid: ' . implode(',', $difference));
        }
    }
}
