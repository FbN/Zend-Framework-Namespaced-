...
class Audioscrobbler
{
     ...
     */
    public function setHttpClient(Client\Client $client)
    {
        $this->_client = $client;
    }
    ...
    protected function _getInfo($service, $params = null)
    {
        ...
        if (preg_match('/No such path/', $responseBody)) {
            ...
            throw new Client\Exception('Could not find: ' . $this->_client->getUri());
        } elseif (preg_match('/No user exists with this name/', $responseBody)) {
            /**
             * @see Zend_Http_Client_Exception
             */
            require_once 'Zend/Http/Client/Exception.php';
            throw new Client\Exception('No user exists with this name');
        } elseif (!$response->isSuccessful()) {
            /**
             * @see Zend_Http_Client_Exception
             */
            require_once 'Zend/Http/Client/Exception.php';
            throw new Client\Exception('The web service ' . $this->_client->getUri() . ' returned the following status code: ' . $response->getStatus());
        }
        ...
        if (!$simpleXmlElementResponse = simplexml_load_string($responseBody)) {
            ...
            $exception = new Exception('Response failed to load with SimpleXML');
