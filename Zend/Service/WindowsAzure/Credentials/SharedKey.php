    ...
    extends CredentialsAbstract
{
	...
	public function signRequestHeaders(
		...
		// Table storage?
		if ($forTableStorage) {
			throw new Exception('The Windows Azure SDK for PHP does not support SharedKey authentication on table storage. Use SharedKeyLite authentication instead.');
		}
		...
		$contentLength = '';
		if (strtoupper($httpVerb) != Client\Client::GET
			 && strtoupper($httpVerb) != Client\Client::DELETE
			 && strtoupper($httpVerb) != Client\Client::HEAD) {
			$contentLength = 0;
