...
class Blob extends Storage\Storage
{
	 ...
	 */
	public function __construct($host = Storage\StorageStorage\Storage::URL_DEV_BLOB, $accountName = Credentials\CredentialsAbstractCredentials\CredentialsAbstract::DEVSTORE_ACCOUNT, $accountKey = Credentials\CredentialsAbstractCredentials\CredentialsAbstract::DEVSTORE_KEY, $usePathStyleUri = false, WindowsAzure\RetryPolicy\RetryPolicyAbstract $retryPolicy = null)
	{
	...
	public function blobExists($containerName = '', $blobName = '', $snapshotId = null)
	{
		...
		} catch (WindowsAzure\Exception $e) {
			return false;
		}
	...
	public function createContainer($containerName = '', $metadata = array())
	{
		...
		// Perform request
		$response = $this->_performRequest($containerName, '?restype=container', Client\Client::PUT, $headers, false, null, Storage\Storage::RESOURCE_CONTAINER, Credentials\CredentialsAbstract::PERMISSION_WRITE);
	...
	public function getContainerAcl($containerName = '', $signedIdentifiers = false)
	{
		...
		// Perform request
		$response = $this->_performRequest($containerName, '?restype=container&comp=acl', Client\Client::GET, array(), false, null, Storage\Storage::RESOURCE_CONTAINER, Credentials\CredentialsAbstract::PERMISSION_READ);
		if ($response->isSuccessful()) {
			if ($signedIdentifiers == false)  {
				// Only private/blob/container
				$accessType = $response->getHeader(Storage\Storage::PREFIX_STORAGE_HEADER . 'blob-public-access');
	...
	public function setContainerAcl($containerName = '', $acl = self::ACL_PRIVATE, $signedIdentifiers = array())
	{
		...
		if ($acl != self::ACL_PRIVATE && $acl !== null && $acl != '') {
			$headers[Storage\Storage::PREFIX_STORAGE_HEADER . 'blob-public-access'] = $acl;
		}
		...
		// Perform request
		$response = $this->_performRequest($containerName, '?restype=container&comp=acl', Client\Client::PUT, $headers, false, $policies, Storage\Storage::RESOURCE_CONTAINER, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if (!$response->isSuccessful()) {
			throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
	}
	...
	public function getContainer($containerName = '')
	{
		...
		// Perform request
		$response = $this->_performRequest($containerName, '?restype=container', Client\Client::GET, array(), false, null, Storage\Storage::RESOURCE_CONTAINER, Credentials\CredentialsAbstract::PERMISSION_READ);
		if ($response->isSuccessful()) {
			// Parse metadata
	...
	public function setContainerMetadata($containerName = '', $metadata = array(), $additionalHeaders = array())
	{
		...
		// Perform request
		$response = $this->_performRequest($containerName, '?restype=container&comp=metadata', Client\Client::PUT, $headers, false, null, Storage\Storage::RESOURCE_CONTAINER, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if (!$response->isSuccessful()) {
			throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
	}
	...
	public function deleteContainer($containerName = '', $additionalHeaders = array())
	{
		...
		// Perform request
		$response = $this->_performRequest($containerName, '?restype=container', Client\Client::DELETE, $headers, false, null, Storage\Storage::RESOURCE_CONTAINER, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if (!$response->isSuccessful()) {
			throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
	}
	...
	public function listContainers($prefix = null, $maxResults = null, $marker = null, $include = null, $currentResultCount = 0)
	{
		...
		// Perform request
		$response = $this->_performRequest('', $queryString, Client\Client::GET, array(), false, null, Storage\Storage::RESOURCE_CONTAINER, Credentials\CredentialsAbstract::PERMISSION_LIST);
		if ($response->isSuccessful()) {
	...
	public function putBlobData($containerName = '', $blobName = '', $data = '', $metadata = array(), $leaseId = null, $additionalHeaders = array())
	{
		...
		// Specify blob type
		$headers[Storage\Storage::PREFIX_STORAGE_HEADER . 'blob-type'] = self::BLOBTYPE_BLOCK;

		// Resource name
		$resourceName = self::createResourceName($containerName , $blobName);

		// Perform request
		$response = $this->_performRequest($resourceName, '', Client\Client::PUT, $headers, false, $data, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if ($response->isSuccessful()) {
			return new nstance(
			$containerName,
			$blobName,
	...
	public function putBlock($containerName = '', $blobName = '', $identifier = '', $contents = '', $leaseId = null)
	{
		...
		// Upload
		$response = $this->_performRequest($resourceName, '?comp=block&blockid=' . base64_encode($identifier), Client\Client::PUT, $headers, false, $contents, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if (!$response->isSuccessful()) {
			throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
	}
	...
	public function putBlockList($containerName = '', $blobName = '', $blockList = array(), $metadata = array(), $leaseId = null, $additionalHeaders = array())
	{
		...
		$blocks,
			...
			// Perform request
			$response = $this->_performRequest($resourceName, '?comp=blocklist', Client\Client::PUT, $headers, false, $fileContents, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
			if (!$response->isSuccessful()) {
				throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
			}
	}
	...
	public function getBlockList($containerName = '', $blobName = '', $snapshotId = null, $leaseId = null, $type = 0)
	{
		...
		// Perform request
		$response = $this->_performRequest($resourceName, $queryString, Client\Client::GET, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_READ);
		if ($response->isSuccessful()) {
			// Parse response
	...
	public function createPageBlob($containerName = '', $blobName = '', $size = 0, $metadata = array(), $leaseId = null, $additionalHeaders = array())
	{
		...
		if ($size <= 0) {
			throw new WindowsAzure\Exception('Page blob size must be specified.');
		}
		...
		$headers[Storage\Storage::PREFIX_STORAGE_HEADER . 'blob-type'] = self::BLOBTYPE_PAGE;
		$headers[Storage\Storage::PREFIX_STORAGE_HEADER . 'blob-content-length'] = $size;
		...
		// Perform request
		$response = $this->_performRequest($resourceName, '', Client\Client::PUT, $headers, false, '', Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if ($response->isSuccessful()) {
			return new nstance(
			$containerName,
			$blobName,
	...
	public function putPage($containerName = '', $blobName = '', $startByteOffset = 0, $endByteOffset = 0, $contents = '', $writeMethod = self::PAGE_WRITE_UPDATE, $leaseId = null, $additionalHeaders = array())
	{
		...
		if ($startByteOffset % 512 != 0) {
			throw new WindowsAzure\Exception('Start byte offset must be a modulus of 512.');
		}
		if (($endByteOffset + 1) % 512 != 0) {
			throw new WindowsAzure\Exception('End byte offset must be a modulus of 512 minus 1.');
		}

		// Determine size
		$size = strlen($contents);
		if ($size >= self::MAX_BLOB_TRANSFER_SIZE) {
			throw new WindowsAzure\Exception('Page blob size must not be larger than ' + self::MAX_BLOB_TRANSFER_SIZE . ' bytes.');
		}
		...
		// Write method
		$headers[Storage\Storage::PREFIX_STORAGE_HEADER . 'page-write'] = $writeMethod;

		// Resource name
		$resourceName = self::createResourceName($containerName , $blobName);

		// Perform request
		$response = $this->_performRequest($resourceName, '?comp=page', Client\Client::PUT, $headers, false, $contents, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if (!$response->isSuccessful()) {
			throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
	}
	...
	public function getPageRegions($containerName = '', $blobName = '', $startByteOffset = 0, $endByteOffset = 0, $leaseId = null)
	{
		...
		// Perform request
		$response = $this->_performRequest($resourceName, '?comp=pagelist', Client\Client::GET, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if ($response->isSuccessful()) {
			$result = $this->_parseResponse($response);
			$xmlRanges = null;
			...
			for ($i = 0; $i < count($xmlRanges); $i++) {
				$ranges[] = new WindowsAzure\Storage\PageRegionInstance(
	...
	public function copyBlob($sourceContainerName = '', $sourceBlobName = '', $destinationContainerName = '', $destinationBlobName = '', $metadata = array(), $sourceSnapshotId = null, $destinationLeaseId = null, $additionalHeaders = array())
	{
		...
		// Perform request
		$response = $this->_performRequest($destinationResourceName, '', Client\Client::PUT, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if ($response->isSuccessful()) {
			return new nstance(
	...
	public function getBlobInstance($containerName = '', $blobName = '', $snapshotId = null, $leaseId = null, $additionalHeaders = array())
	{
		...
		// Perform request
		$response = $this->_performRequest($resourceName, $queryString, Client\Client::HEAD, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_READ);
		if ($response->isSuccessful()) {
			// Parse metadata
			$metadata = $this->_parseMetadataHeaders($response->getHeaders());
	...
	public function setBlobMetadata($containerName = '', $blobName = '', $metadata = array(), $leaseId = null, $additionalHeaders = array())
	{
		...
		// Perform request
		$response = $this->_performRequest($containerName . '/' . $blobName, '?comp=metadata', Client\Client::PUT, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if (!$response->isSuccessful()) {
			throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
	}
	...
	public function setBlobProperties($containerName = '', $blobName = '', $leaseId = null, $additionalHeaders = array())
	{
		...
		if (count($additionalHeaders) == 0) {
			throw new WindowsAzure\Exception('No additional headers are specified.');
		}
		...
		// Perform request
		$response = $this->_performRequest($containerName . '/' . $blobName, '?comp=properties', Client\Client::PUT, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if (!$response->isSuccessful()) {
			throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
	}
	...
	public function deleteBlob($containerName = '', $blobName = '', $snapshotId = null, $leaseId = null, $additionalHeaders = array())
	{
		...
		// Perform request
		$response = $this->_performRequest($resourceName, $queryString, Client\Client::DELETE, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if (!$response->isSuccessful()) {
			throw new WindowsAzure\Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
	}
	...
	public function snapshotBlob($containerName = '', $blobName = '', $metadata = array(), $additionalHeaders = array())
	{
		...
		// Perform request
		$response = $this->_performRequest($resourceName, '?comp=snapshot', Client\Client::PUT, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if ($response->isSuccessful()) {
	...
	public function leaseBlob($containerName = '', $blobName = '', $leaseAction = self::LEASE_ACQUIRE, $leaseId = null)
	{
		...
		// Perform request
		$response = $this->_performRequest($resourceName, '?comp=lease', Client\Client::PUT, $headers, false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_WRITE);
		if ($response->isSuccessful()) {
			return new WindowsAzure\Storage\LeaseInstance(
			$containerName,
			$blobName,
	...
	public function listBlobs($containerName = '', $prefix = '', $delimiter = '', $maxResults = null, $marker = null, $include = null, $currentResultCount = 0)
	{
		...
		// Perform request
		$response = $this->_performRequest($containerName, $queryString, Client\Client::GET, array(), false, null, Storage\Storage::RESOURCE_BLOB, Credentials\CredentialsAbstract::PERMISSION_LIST);
		if ($response->isSuccessful()) {
			// Return value
			$blobs = array();
