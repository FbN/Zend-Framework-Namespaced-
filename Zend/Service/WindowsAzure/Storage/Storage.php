...
class Storage
{
	...
	public function __construct(
		...
		$accountName = Credentials\CredentialsAbstractCredentials\CredentialsAbstract::DEVSTORE_ACCOUNT,
		$accountKey = Credentials\CredentialsAbstractCredentials\CredentialsAbstract::DEVSTORE_KEY,
		$usePathStyleUri = false,
		RetryPolicy\RetryPolicyAbstract $retryPolicy = null
	) {
		$this->_host = $host;
		...
		if ($this->_retryPolicy === null) {
		    $this->_retryPolicy = RetryPolicy\RetryPolicyAbstract::noRetry();
		}
		...
		$this->_httpClientChannel = new Client\Client(null, $options);
	}
	 ...
	 * @param Zend_Service_WindowsAzure_RetryPolicy_RetryPolicyAbstract $retryPolicy Retry policy to use when making requests
	 */
	public function setRetryPolicy(RetryPolicy\RetryPolicyAbstract $retryPolicy = null)
	{
		$this->_retryPolicy = $retryPolicy;
		if ($this->_retryPolicy === null) {
		    $this->_retryPolicy = RetryPolicy\RetryPolicyAbstract::noRetry();
	 ...
	 */
	public function setCredentials(Credentials\CredentialsAbstract $credentials)
	{
	...
	protected function _performRequest(
		...
		$requiredPermission = Credentials\CredentialsAbstract::PERMISSION_READ
	) {
	    // Clean path
		if (strpos($path, '/') !== 0) {
			$path = '/' . $path;
	 ...
	 */
	protected function _parseResponse(\Zend\Http\Response\Response $response = null)
	{
		if ($response === null) {
	...
	protected function _generateMetadataHeaders($metadata = array())
	{
		...
		foreach ($metadata as $key => $value) {
			...
			if (!self::isValidMetadataName($key)) {
		    	throw new WindowsAzure\Exception('Metadata name does not adhere to metadata naming conventions. See http://msdn.microsoft.com/en-us/library/aa664670(VS.71).aspx for more information.');
			}
