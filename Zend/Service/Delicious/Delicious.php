...
class Delicious
{
    ...
    {
        $this->_rest = new \Zend\Rest\Client\Client();
    ...
    public function getLastUpdate()
    {
        ...
        if ($rootNode && $rootNode->nodeName == 'update') {
             ...
             */
            return new Date\Date(strtotime($rootNode->getAttribute('time')));
        } else {
            /**
     ...
     */
    public function getPosts($tag = null, Date\Date $dt = null, $url = null)
    {
        $parms = array();
        if ($tag) {
            $parms['tag'] = $tag;
