...
class SimpleDb extends \Zend\Service\Amazon\AbstractAmazon
{
    ...
    {
    	if(!($endpoint instanceof \Zend\Uri\Http)) {
    ...
    public function quoteName($name)
    {
    	if (preg_match('/^[a-z_$][a-z0-9_$-]*$/i', $name) == false) {
    		throw new Exception("Invalid name: can contain only alphanumeric characters, \$ and _");
    	}
    	return "`$name`";
    }
   ...
   /**
    ...
    protected function _sendRequest(array $params = array())
    {
        ...
        try {
            ...
            $request->setMethod(Client\Client::POST);
            foreach ($params as $key => $value) {
                $params_out[] = rawurlencode($key)."=".rawurlencode($value);
            }
            $request->setRawData(join('&', $params_out), Client\Client::ENC_URLENCODED);
            $httpResponse = $request->request();
        } catch (Client\Exception $zhce) {
    ...
    protected function _signParameters(array $paramaters)
    {
        ...
        require_once 'Zend/Crypt/Hmac.php';
        $hmac = Hmac\Hmac::compute($this->_getSecretKey(), 'SHA256', $data, Hmac\Hmac::BINARY);
