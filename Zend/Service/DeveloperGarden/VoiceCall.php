    ...
    extends Client\ClientAbstract
{
    ...
    public function newCall($aNumber, $bNumber, $expiration, $maxDuration,
        ...
        $request = new Request\VoiceButler\NewCall(
                $this->getEnvironment()
        );
    ...
    public function newCallSequenced($aNumber, $bNumber, $expiration, $maxDuration,
        ...
        $request = new Request\VoiceButler\NewCallSequenced(
                $this->getEnvironment()
        );
        $request->setANumber($aNumber)
                ->setBNumber($bNumber)
    ...
    {
        $request = new Request\VoiceButler\TearDownCall(
            $this->getEnvironment(),
            $sessionId
        );
    ...
    {
        $request = new Request\VoiceButler\CallStatus(
            $this->getEnvironment(),
            $sessionId,
            $keepAlive
        );
