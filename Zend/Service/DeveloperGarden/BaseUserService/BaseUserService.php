 ...
 * @namespace
 */
namespace Zend\Service\DeveloperGarden\BaseUserService;
use Zend\Service\DeveloperGarden\Client;
use Zend\Service\DeveloperGarden\Request\BaseUserService;
use Zend\Service\DeveloperGarden\Request\BaseUserService;
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class BaseUserService extends Client\ClientAbstract
{
    /**
     * wsdl file
     *
    ...
    protected function _checkModuleId($moduleId)
    {
        if (!in_array($moduleId, $this->_moduleIds)) {
            ...
            throw new Client\Exception('moduleId not valid');
        }
    }
    ...
    {
        return new BaseUserService\GetQuotaInformation(
            $moduleId
        );
    }
    ...
    {
        return new BaseUserService\ChangeQuotaPool(
            $moduleId,
            $quotaMax
        );
    }
    ...
    public function getQuotaInformation(
        BaseUserService\GetQuotaInformation $request
    ) {
    ...
    public function changeQuotaPool(
        BaseUserService\ChangeQuotaPool $request
    ) {
        $this->_checkModuleId($request->getModuleId());
        return $this->getSoapClient()
    ...
    {
        $request = new BaseUserService\GetAccountBalance(
            $accounts
        );
        return $this->getSoapClient()
