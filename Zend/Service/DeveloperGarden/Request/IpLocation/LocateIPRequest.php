 ...
 * @namespace
 */
namespace Zend\Service\DeveloperGarden\Request\IpLocation;
use Zend\Service\DeveloperGarden\IpLocation;
use Zend\Service\DeveloperGarden\Request;
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class LocateIPRequest
    extends Request\RequestAbstract
{
    /**
    ...
    {
        if ($ip instanceof IpLocation\IpAddress) {
            $this->address[] = array(
        ...
        if (is_array($ip)) {
            foreach ($ip as $ipObject) {
                if (!$ipObject instanceof IpLocation\IpAddress
                    && !is_string($ipObject)
                ) {
        ...
        if (!is_string($ip)) {
            require_once 'Zend/Service/DeveloperGarden/Request/Exception.php';
            throw new Request\Exception('Not a valid Ip Address object found.');
        }

        return $this->setIp(new IpLocation\IpAddress($ip));
    }
}
