    ...
    extends Client\ClientAbstract
{
    ...
    {
        $request = new Request\SendSms\SendSMS($this->getEnvironment());
    ...
    {
        $request = new Request\SendSms\SendFlashSMS($this->getEnvironment());
        $request->setNumber($number)
                ->setMessage($message)
                ->setOriginator($originator)
                ->setAccount($account);
     ...
     */
    public function send(Request\SendSms\SendSmsAbstract $sms)
    {
        ...
        switch ($sms->getSmsType()) {
            ...
            default : {
                ...
                throw new Client\Exception('Unknown SMS Type');
            }
        }
