 ...
 * @since      LiveDocx 1.0 
 */
class MailMerge extends LiveDocx
{
    /**
    ...
    protected function _backendListArrayToMultiAssocArray($list)
    {
        ...
        if (isset($list->ArrayOfString)) {
           foreach ($list->ArrayOfString as $a) {
               ...
               if (isset($o->string)) {
                   $date1 = new Date\Date($o->string[3], Date\Date::RFC_1123);
                   $date2 = new Date\Date($o->string[1], Date\Date::RFC_1123);

                   $ret[] = array (
                        'filename'   => $o->string[0],
                        'fileSize'   => (integer) $o->string[2],
                        'createTime' => (integer) $date1->get(Date\Date::TIMESTAMP),
                        'modifyTime' => (integer) $date2->get(Date\Date::TIMESTAMP),
                   );
               }
           }
        }
