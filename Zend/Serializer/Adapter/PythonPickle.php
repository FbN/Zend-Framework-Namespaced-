 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class PythonPickle extends AdapterAbstract
{
    ...
    protected function _checkProtocolNumber($number)
    {
        ...
        if ($int < 0 || $int > 3) {
            ...
            throw new Serializer\Exception('Invalid or unknown protocol version "'.$number.'"');
        }
        return $int;
    }
    ...
    protected function _write($value)
    {
        if ($value === null) {
            ...
            require_once 'Zend/Serializer/Exception.php';
            throw new Serializer\Exception(
    ...
    public function unserialize($pickle, array $opts = array())
    {
        ...
        if (!count($this->_stack)) {
            require_once 'Zend/Serializer/Exception.php';
            throw new Serializer\Exception('No data found');
        }
    ...
    protected function _load($op)
    {
        switch ($op) {
            ...
            default:
                require_once 'Zend/Serializer/Exception.php';
                throw new Serializer\Exception('Invalid or unknown opcode "'.$op.'"');
        }
    }
    ...
    protected function _loadPut()
    {
        ...
        if (!isset($this->_stack[$lastStack])) {
            require_once 'Zend/Serializer/Exception.php';
            throw new Serializer\Exception('No stack exist');
        }
    ...
    protected function _loadGet()
    {
        ...
        if (!array_key_exists($id, $this->_memo)) {
            require_once 'Zend/Serializer/Exception.php';
            throw new Serializer\Exception('Get id "' . $id . '" not found in momo');
        }
    ...
    protected function _hex2Utf8($hex)
    {
        ...
        if ($uniCode < 0x80) { // 1Byte
            ...
            require_once 'Zend/Serializer/Exception.php';
            throw new Serializer\Exception('Unsupported unicode character found "' . dechex($uniCode) . '"');
        }
    ...
    protected function _loadProto()
    {
        ...
        if ($proto < 2 || $proto > 3) {
            require_once 'Zend/Serializer/Exception.php';
            throw new Serializer\Exception('Invalid protocol version detected');
        }
    ...
    protected function _read($len)
    {
        if (($this->_pos + $len) > $this->_pickleLen) {
            require_once 'Zend/Serializer/Exception.php';
            throw new Serializer\Exception('End of data');
        }
    ...
    protected function _readline()
    {
        ...
        if ($eolPos === false) {
            require_once 'Zend/Serializer/Exception.php';
            throw new Serializer\Exception('No new line found');
        }
