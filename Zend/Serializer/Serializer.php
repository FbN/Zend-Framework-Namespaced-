...
class Serializer
{
    ...
    {
        if ($adapterName instanceof Adapter\AdapterInterface) {
        ...
        if (!in_array('Zend_Serializer_Adapter_AdapterInterface', class_implements($adapterClass))) {
            require_once 'Zend/Serializer/Exception.php';
            throw new Exception('The serializer adapter class "'.$adapterClass.'" must implement Zend_Serializer_Adapter_AdapterInterface');
        }
    ...
    {
        if (!self::$_defaultAdapter instanceof Adapter\AdapterInterface) {
