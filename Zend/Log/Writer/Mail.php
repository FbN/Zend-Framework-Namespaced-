...
class Mail extends AbstractWriter
{
     ...
     * @return void
     */
    public function __construct(Mail\Mail $mail, Layout\Layout $layout = null)
    {
        $this->_mail = $mail;
        if (null !== $layout) {
    ...
    public function setLayout($layout)
    {
        ...
        if (!$layout instanceof Layout\Layout) {
            require_once 'Zend/Log/Exception.php';
            throw new Log\Exception('Mail must be an instance of Zend_Layout or an array');
        }
        $this->_layout = $layout;
    ...
    protected static function _constructMailFromConfig(array $config)
    {
        ...
        if (!$mail \instanceof Mail\Mail) {
            throw new Log\Exception($mail . 'must extend Zend_Mail');
        }
    ...
    protected function _constructLayoutFromConfig(array $config)
    {
        ...
        if (!$layout \instanceof Layout\Layout) {
            throw new Log\Exception($layout . 'must extend Zend_Layout');
        }
     ...
     * @throws Zend_Log_Exception
     */
    public function setLayoutFormatter(Log\Formatter\FormatterInterface $formatter)
    {
        if (!$this->_layout) {
