 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class DeviceAtlas implements Adapter
{
    /**
    ...
    public static function getFromRequest($request, array $config)
    {
        if (!class_exists('Mobi_Mtld_DA_Api')) {
            if (!isset($config['deviceatlas'])) {
                ...
                throw new Features\Exception('"DeviceAtlas" configuration is not defined');
            }
        }
        ...
        if (!class_exists('Mobi_Mtld_DA_Api')) {
            if (empty($config['deviceatlas_lib_dir'])) {
                require_once 'Zend/Http/UserAgent/Features/Exception.php';
                throw new Features\Exception('The "deviceatlas_lib_dir" parameter is not defined');
            }
        ...
        if (empty($config['deviceatlas_data'])) {
            require_once 'Zend/Http/UserAgent/Features/Exception.php';
            throw new Features\Exception('The "deviceatlas_data" parameter is not defined');
        }
        
        //load the device data-tree : e.g. 'json/DeviceAtlas.json
        $tree = \Mobi\Mtld\DA\Api::getTreeFromFile($config['deviceatlas_data']);
        
        $properties = \Mobi\Mtld\DA\Api::getProperties($tree, $request['http_user_agent']);
