 ...
 * @namespace
 */
namespace Zend\Http\UserAgent\Features\Adapter;
use Zend\Http\UserAgent\Features;
    ...
    implements Adapter 
{
    ...
    public static function getFromRequest($request, array $config)
    {
        if (!isset($config['wurflapi'])) {
            ...
            throw new Features\Exception('"wurflapi" configuration is not defined');
        }
        ...
        if (empty($config['wurfl_lib_dir'])) {
            require_once 'Zend/Http/UserAgent/Features/Exception.php';
            throw new Features\Exception('The "wurfl_lib_dir" parameter is not defined');
        }
        if (empty($config['wurfl_config_file']) && empty($config['wurfl_config_array'])) {
            require_once 'Zend/Http/UserAgent/Features/Exception.php';
            throw new Features\Exception('The "wurfl_config_file" parameter is not defined');
        }
        ...
        switch ($config['wurfl_api_version']) {
            case '1.0':
                ...
                $wurflManager = \WURFL\WURFLManagerProvider::getWURFLManager(\Zend\Http\UserAgent\UserAgent::$config['wurfl_config_file']);
                break;
            case '1.1':
                require_once ($config['wurfl_lib_dir'] . 'Application.php');
                if (!empty($config['wurfl_config_file'])) {
                    $wurflConfig = \WURFL\Configuration\ConfigFactory::create($config['wurfl_config_file']);
            ...
            default:
                require_once 'Zend/Http/UserAgent/Features/Exception.php';
                throw new Features\Exception(sprintf(
