...
class Autoloader
{
    ...
    public function setDefaultAutoloader($callback)
    {
        if (!is_callable($callback)) {
            throw new Loader\Exception('Invalid callback specified for default autoloader');
        }
    ...
    public function registerNamespace($namespace)
    {
        if (is_string($namespace)) {
            ...
            throw new Loader\Exception('Invalid namespace provided');
        }
    ...
    public function setZfPath($spec, $version = 'latest')
    {
        ...
        if (is_array($spec)) {
            if (!isset($spec['path'])) {
                throw new Loader\Exception('No path specified for ZF');
            }
            $path = $spec['path'];
    ...
    protected function _getVersionPath($path, $version)
    {
        ...
        if (empty($availableVersions)) {
            throw new Loader\Exception('No valid ZF installations discovered');
        }
    ...
    protected function _getVersionType($version)
    {
        ...
        if (3 < $count) {
            throw new Loader\Exception('Invalid version string provided');
        }
        return 'specific';
    }
    ...
    {
        if (!is_dir($path)) {
            throw new Loader\Exception('Invalid ZF path provided');
        }
