 ...
 * @namespace
 */
namespace Zend\Loader\Autoloader;
use Zend\Loader;
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Resource implements AutoloaderInterface
{
    /**
    ...
    public function __construct($options)
    {
        ...
        if (!is_array($options)) {
            ...
            throw new Loader\Exception('Options must be passed to resource loader constructor');
        }
        ...
        if ((null === $namespace)
            ...
            require_once 'Zend/Loader/Exception.php';
            throw new Loader\Exception('Resource loader requires both a namespace and a base path for initialization');
        }
    ...
    public function __call($method, $args)
    {
        if ('get' == substr($method, 0, 3)) {
            ...
            if (!$this->hasResourceType($type)) {
                require_once 'Zend/Loader/Exception.php';
                throw new Loader\Exception("Invalid resource type $type; cannot load resource");
            }
            if (empty($args)) {
                require_once 'Zend/Loader/Exception.php';
                throw new Loader\Exception("Cannot load resources; no resource specified");
            }
        ...
        require_once 'Zend/Loader/Exception.php';
        throw new Loader\Exception("Method '$method' is not supported");
    }
    ...
    public function getClassPath($class)
    {
        ...
        if (Loader\Loader::isReadable($classPath)) {
            return $classPath;
        }
    ...
    public function addResourceType($type, $path, $namespace = null)
    {
        ...
        if (!isset($this->_resourceTypes[$type])) {
            if (null === $namespace) {
                require_once 'Zend/Loader/Exception.php';
                throw new Loader\Exception('Initial definition of a resource type must include a namespace');
            }
            $namespaceTopLevel = $this->getNamespace();
        ...
        if (!is_string($path)) {
            require_once 'Zend/Loader/Exception.php';
            throw new Loader\Exception('Invalid path specification provided; must be string');
        }
    ...
    public function addResourceTypes(array $types)
    {
        foreach ($types as $type => $spec) {
            if (!is_array($spec)) {
                require_once 'Zend/Loader/Exception.php';
                throw new Loader\Exception('addResourceTypes() expects an array of arrays');
            }
            if (!isset($spec['path'])) {
                require_once 'Zend/Loader/Exception.php';
                throw new Loader\Exception('addResourceTypes() expects each array to include a paths element');
            }
            $paths  = $spec['path'];
            $namespace = null;
    ...
    public function load($resource, $type = null)
    {
        if (null === $type) {
            ...
            if (empty($type)) {
                require_once 'Zend/Loader/Exception.php';
                throw new Loader\Exception('No resource type specified');
            }
        }
        if (!$this->hasResourceType($type)) {
            require_once 'Zend/Loader/Exception.php';
            throw new Loader\Exception('Invalid resource type specified');
        }
        $namespace = $this->_resourceTypes[$type]['namespace'];
