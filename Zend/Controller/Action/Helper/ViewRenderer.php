        ...
        if (null === $this->_inflector) {
            ...
            $this->_inflector->setStaticRuleReference('moduleDir', $this->_moduleDir) // moduleDir must be specified before the less specific 'module'
                 ->addRules(array(
                     ...
                     ':controller' => array('Word_CamelCaseToDash', new Filter\Word\UnderscoreToSeparator('/'), 'StringToLower', new Filter\PregReplace('/\./', '-')),
                     ':action'     => array('Word_CamelCaseToDash', new Filter\PregReplace('#[^a-z0-9' . preg_quote('/', '#') . ']+#i', '-'), 'StringToLower'),
                 ))
