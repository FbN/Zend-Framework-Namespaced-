...
abstract class RendererAbstract
{
    ...
    public function addMarkup($name, $type, array $options)
    {
        ...
        if (isset($options['filter'])) {
            if ($options['filter'] instanceof Filter\FilterInterface) {
    ...
    protected function _execute(Markup\Token $token)
    {
        ...
        if (!isset($markup['filter'])
            || (!($markup['filter'] instanceof Filter\FilterInterface) && ($markup['filter'] !== false))) {
    ...
    {
        if ($this->_filter instanceof Filter\FilterInterface) {
     ...
     * @return void
     */
    public function addDefaultFilter(Filter\FilterInterface $filter, $placement = Filter\FilterFilter\Filter::CHAIN_APPEND)
    {
        if (!($this->_defaultFilter instanceof Filter\Filter)) {
            $defaultFilter = new Filter\Filter();
     ...
     * @return void
     */
    public function setDefaultFilter(Filter\FilterInterface $filter)
    {
    ...
    public function getFilter($markup)
    {
        ...
        if (!isset($this->_markups[$markup]['filter'])
            || !($this->_markups[$markup]['filter'] instanceof Filter\FilterInterface)
        ) {
     ...
     * @return Zend_Markup_Renderer_RendererAbstract
     */
    public function addFilter(Filter\FilterInterface $filter, $markup, $placement = Filter\FilterFilter\Filter::CHAIN_APPEND)
    {
        $markup = $this->_resolveMarkupName($markup);
        ...
        if (!($oldFilter instanceof Filter\Filter)) {
            $this->_markups[$markup]['filter'] = new Filter\Filter();

            if ($oldFilter instanceof Filter\FilterInterface) {
     ...
     * @return Zend_Markup_Renderer_RendererAbstract
     */
    public function setFilter(Filter\FilterInterface $filter, $markup)
    {
        $markup = $this->_resolveMarkupName($markup);
