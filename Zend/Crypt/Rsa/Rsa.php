...
class Rsa
{
    ...
    public function __construct(array $options = null)
    {
        if (!extension_loaded('openssl')) {
            ...
            throw new Exception('Zend_Crypt_Rsa requires openssl extention to be loaded.');
        }
     ...
     * @return string
     */
    public function sign($data, Key\Private $privateKey = null, $format = null)
    {
        $signature = '';
        if (isset($privateKey)) {
     ...
     * @return string
     */
    public function encrypt($data, Key\Key $key, $format = null)
    {
        $encrypted = '';
     ...
     * @return string
     */
    public function decrypt($data, Key\Key $key, $format = null)
    {
        $decrypted = '';
        if ($format == self::BASE64) {
    ...
    public function generateKeys(array $configargs = null)
    {
        ...
        $privateKey = new Key\Private($private, $passPhrase);
        $details = openssl_pkey_get_details($resource);
        $publicKey = new Key\Public($details['key']);
    ...
    public function setPemString($value)
    {
        ...
        try {
            $this->_privateKey = new Key\Private($this->_pemString, $this->_passPhrase);
            $this->_publicKey = $this->_privateKey->getPublicKey();
        } catch (\Zend\Crypt\Exception $e) {
            $this->_privateKey = null;
            $this->_publicKey = new Key\Public($this->_pemString);
        }
    }
    ...
    public function setCertificateString($value)
    {
        ...
        $this->_publicKey = new Key\Public($this->_certificateString, $this->_passPhrase);
    }
