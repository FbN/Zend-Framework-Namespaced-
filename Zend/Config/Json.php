...
class Json extends Config
{
    ...
    public function __construct($json, $section = null, $options = false)
    {
        ...
        // Parse/decode
        $config = \Zend\Json\Json::decode($json);
        ...
        if ($section === null) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($config[$sectionName])) {
                    require_once 'Zend/Config/Exception.php';
                    throw new Exception(sprintf('Section "%s" cannot be found', $sectionName));
                }
            ...
            if (!isset($config[$section])) {
                require_once 'Zend/Config/Exception.php';
                throw new Exception(sprintf('Section "%s" cannot be found', $section));
            }
    ...
    protected function _processExtends(array $data, $section, array $config = array())
    {
        ...
        if (is_array($thisSection) && isset($thisSection[self::EXTENDS_NAME])) {
            if (is_array($thisSection[self::EXTENDS_NAME])) {
                require_once 'Zend/Config/Exception.php';
                throw new Exception('Invalid extends clause: must be a string; array received');
            }
