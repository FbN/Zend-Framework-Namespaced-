...
class Json extends FileAbstract
{
    ...
    public function render()
    {
        ...
        foreach ($extends as $section => $parentSection) {
            $data[$section][Config\Json::EXTENDS_NAME] = $parentSection;
        }

        // Ensure that each "extends" section actually exists
        foreach ($data as $section => $sectionData) {
            if (is_array($sectionData) && isset($sectionData[Config\Json::EXTENDS_NAME])) {
                $sectionExtends = $sectionData[Config\Json::EXTENDS_NAME];
                if (!isset($data[$sectionExtends])) {
                    // Remove "extends" declaration if section does not exist
                    unset($data[$section][Config\Json::EXTENDS_NAME]);
                }
            }
        }

        $out = Json\Json::encode($data);
        if ($this->prettyPrint()) {
             $out = Json\Json::prettyPrint($out);
        }
        return $out;
    }
}
