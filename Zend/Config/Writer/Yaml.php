...
class Yaml extends FileAbstract
{
    ...
    public function setYamlEncoder($yamlEncoder)
    {
        if (!is_callable($yamlEncoder)) {
            ...
            throw new Config\Exception('Invalid parameter to setYamlEncoder - must be callable');
        }
    ...
    public function render()
    {
        ...
        foreach ($extends as $section => $parentSection) {
            $data[$section][Config\Yaml::EXTENDS_NAME] = $parentSection;
        }

        // Ensure that each "extends" section actually exists
        foreach ($data as $section => $sectionData) {
            if (is_array($sectionData) && isset($sectionData[Config\Yaml::EXTENDS_NAME])) {
                $sectionExtends = $sectionData[Config\Yaml::EXTENDS_NAME];
                if (!isset($data[$sectionExtends])) {
                    // Remove "extends" declaration if section does not exist
                    unset($data[$section][Config\Yaml::EXTENDS_NAME]);
                }
            }
        }
