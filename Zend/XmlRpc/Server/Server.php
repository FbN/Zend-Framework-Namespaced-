 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Server extends Server\AbstractServer
{
    /**
     * Character encoding
     * @var string
    ...
    {
        $this->_table = new Server\Definition();
    ...
    public function __call($method, $params)
    {
        ...
        if (!method_exists($system, $method)) {
            ...
            throw new Exception('Unknown instance method called on server: ' . $method);
        }
    ...
    public function addFunction($function, $namespace = '')
    {
        if (!is_string($function) && !is_array($function)) {
            require_once 'Zend/XmlRpc/Server/Exception.php';
            throw new Exception('Unable to attach function; invalid', 611);
        }
    ...
    public function setClass($class, $namespace = '', $argv = null)
    {
        if (is_string($class) && !class_exists($class)) {
            require_once 'Zend/XmlRpc/Server/Exception.php';
            throw new Exception('Invalid method class', 610);
        }
    ...
    {
        // Get request
        if ((!$request || !$request instanceof Request\Request)
    ...
    {
        if (!is_array($definition) && (!$definition instanceof Server\Definition)) {
            ...
            require_once 'Zend/XmlRpc/Server/Exception.php';
            throw new Exception('Unable to load server definition; must be an array or Zend_Server_Definition, received ' . $type, 612);
        }

        $this->_table->clearMethods();
        $this->_registerSystemMethods();

        if ($definition instanceof Server\Definition) {
    ...
    public function setEncoding($encoding)
    {
        ...
        Value\Value::setEncoding($encoding);
        return $this;
    }
    ...
    protected function _handle(Request\Request $request)
    {
        ...
        if (!$this->_table->hasMethod($method)) {
            require_once 'Zend/XmlRpc/Server/Exception.php';
            throw new Exception('Method "' . $method . '" does not exist', 620);
        }
        ...
        if ($sigLength < $paramsLen) {
            for ($i = $sigLength; $i < $paramsLen; ++$i) {
                $xmlRpcValue = Value\Value::getXmlRpcValue($params[$i]);
        ...
        if (!$matched) {
            require_once 'Zend/XmlRpc/Server/Exception.php';
            throw new Exception('Calling parameters do not match signature', 623);
        }
