...
class Response
{
    ...
    {
        return Value\Value::getXmlRpcValue($this->_return);
    }
    ...
    public function loadXml($response)
    {
        if (!is_string($response)) {
            $this->_fault = new XmlRpc\Fault(650);
        ...
        try {
            ...
            libxml_use_internal_errors($useInternalXmlErrors);
            // Not valid XML
            $this->_fault = new XmlRpc\Fault(651);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
        ...
        if (empty($xml->params)) {
            // Invalid response
            $this->_fault = new XmlRpc\Fault(652);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
        ...
        try {
            ...
            $value = Value\Value::getXmlRpcValue($valueXml, Value\Value::XML_STRING);
        } catch (Value\Exception $e) {
            $this->_fault = new XmlRpc\Fault(653);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
