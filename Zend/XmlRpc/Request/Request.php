...
class Request
{
    ...
    public function setMethod($method)
    {
        if (!is_string($method) || !preg_match('/^[a-z0-9_.:\/]+$/i', $method)) {
            $this->_fault = new XmlRpc\Fault(634, 'Invalid method name ("' . $method . '")');
    ...
    public function addParam($value, $type = null)
    {
        ...
        if (null === $type) {
            ...
            if ($value instanceof Value\Value) {
                ...
                $xmlRpcValue = Value\Value::getXmlRpcValue($value);
    ...
    public function setParams()
    {
        ...
        if ((1 == $argc) && is_array($argv[0])) {
            ...
            foreach ($argv[0] as $arg) {
                ...
                if (!isset($arg['type'])) {
                    $xmlRpcValue = Value\Value::getXmlRpcValue($arg['value']);
            ...
            if ($wellFormed) {
                ...
                foreach ($argv[0] as $arg) {
                    if ($arg instanceof Value\Value) {
                        ...
                        $xmlRpcValue = Value\Value::getXmlRpcValue($arg);
                        $type        = $xmlRpcValue->getType();
                    }
    ...
    public function loadXml($request)
    {
        if (!is_string($request)) {
            $this->_fault = new XmlRpc\Fault(635);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
        ...
        try {
            ...
            // Not valid XML
            $this->_fault = new XmlRpc\Fault(631);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
        ...
        if (empty($xml->methodName)) {
            // Missing method name
            $this->_fault = new XmlRpc\Fault(632);
            $this->_fault->setEncoding($this->getEncoding());
            return false;
        }
        ...
        if (!empty($xml->params)) {
            ...
            foreach ($xml->params->children() as $param) {
                if (!isset($param->value)) {
                    $this->_fault = new XmlRpc\Fault(633);
                    $this->_fault->setEncoding($this->getEncoding());
                    return false;
                }

                try {
                    $param   = Value\Value::getXmlRpcValue($param->value, Value\Value::XML_STRING);
                    $types[] = $param->getType();
                    $argv[]  = $param->getValue();
                } catch (\Exception $e) {
                    $this->_fault = new XmlRpc\Fault(636);
                    $this->_fault->setEncoding($this->getEncoding());
                    return false;
                }
            }
    ...
    {
        return $this->_fault instanceof XmlRpc\Fault;
    }
    ...
    protected function _getXmlRpcParams()
    {
        ...
        if (is_array($this->_xmlRpcParams)) {
            foreach ($this->_xmlRpcParams as $param) {
                $value = $param['value'];
                $type  = isset($param['type']) ? $param['type'] : Value\Value::AUTO_DETECT_TYPE;

                if (!$value instanceof Value\Value) {
                    $value = Value\Value::getXmlRpcValue($value, $type);
                }
                $params[] = $value;
            }
        }
    ...
    public function saveXml()
    {
        ...
        $generator = Value\Value::getGenerator();
