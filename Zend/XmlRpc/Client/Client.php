...
class Client
{
     ...
     * @return void
     */
    public function __construct($server, Client\Client $httpClient = null)
    {
        if ($httpClient === null) {
            $this->_httpClient = new Client\Client();
        } else {
     ...
     */
    public function setHttpClient(Client\Client $httpClient)
    {
   ...
   /**
    ...
    public function doRequest($request, $response = null)
    {
        ...
        $http->setRawData($xml);
        $httpResponse = $http->request(Client\Client::POST);
        ...
        if ($response === null) {
            $response = new \Zend\XmlRpc\Response\Response();
        }
    ...
    public function call($method, $params=array())
    {
        if (!$this->skipSystemLookup() && ('system.' != substr($method, 0, 7))) {
            ...
            if ($success) {
                $validTypes = array(
                    Value\Value::XMLRPC_TYPE_ARRAY,
                    Value\Value::XMLRPC_TYPE_BASE64,
                    Value\Value::XMLRPC_TYPE_BOOLEAN,
                    Value\Value::XMLRPC_TYPE_DATETIME,
                    Value\Value::XMLRPC_TYPE_DOUBLE,
                    Value\Value::XMLRPC_TYPE_I4,
                    Value\Value::XMLRPC_TYPE_INTEGER,
                    Value\Value::XMLRPC_TYPE_NIL,
                    Value\Value::XMLRPC_TYPE_STRING,
                    Value\Value::XMLRPC_TYPE_STRUCT,
                );
                ...
                foreach ($params as $key => $param) {
                    ...
                    $type = Value\Value::AUTO_DETECT_TYPE;
                    ...
                    $params[$key] = Value\Value::getXmlRpcValue($param, $type);
                }
            }
        }
    ...
    {
        return new \Zend\XmlRpc\Request\Request($method, $params);
    }
}
