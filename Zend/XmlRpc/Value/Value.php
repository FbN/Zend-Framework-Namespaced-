...
abstract class Value
{
    ...
    public static function getGenerator()
    {
        if (!self::$_generator) {
            if (extension_loaded('xmlwriter')) {
                ...
                self::$_generator = new \Zend\XmlRpc\Generator\XmlWriter();
            } else {
                require_once 'Zend/XmlRpc/Generator/DomDocument.php';
                self::$_generator = new \Zend\XmlRpc\Generator\DomDocument();
            }
        }
     ...
     * @return void
     */
    public static function setGenerator(\Zend\XmlRpc\Generator\GeneratorAbstract $generator)
    {
    ...
    public static function getXmlRpcValue($value, $type = self::AUTO_DETECT_TYPE)
    {
        switch ($type) {
            ...
            default:
                ...
                throw new Exception('Given type is not a '. __CLASS__ .' constant');
        }
    }
    ...
    protected static function _xmlStringToNativeXmlRpc($xml)
    {
        ...
        switch ($type) {
            ...
            case self::XMLRPC_TYPE_INTEGER:
                require_once 'Zend/XmlRpc/Value/Integer.php';
                $xmlrpcValue = new Integer($value);
                break;
            case self::XMLRPC_TYPE_APACHEI8:
                // Fall through to the next case
            case self::XMLRPC_TYPE_I8:
                require_once 'Zend/XmlRpc/Value/BigInteger.php';
                $xmlrpcValue = new BigInteger($value);
                break;
            case self::XMLRPC_TYPE_DOUBLE:
                require_once 'Zend/XmlRpc/Value/Double.php';
                $xmlrpcValue = new Double($value);
                break;
            case self::XMLRPC_TYPE_BOOLEAN:
                require_once 'Zend/XmlRpc/Value/Boolean.php';
                $xmlrpcValue = new Boolean($value);
                break;
            case self::XMLRPC_TYPE_STRING:
                require_once 'Zend/XmlRpc/Value/String.php';
                $xmlrpcValue = new String($value);
                break;
            case self::XMLRPC_TYPE_DATETIME:  // The value should already be in a iso8601 format
                require_once 'Zend/XmlRpc/Value/DateTime.php';
                $xmlrpcValue = new DateTime($value);
                break;
            case self::XMLRPC_TYPE_BASE64:    // The value should already be base64 encoded
                require_once 'Zend/XmlRpc/Value/Base64.php';
                $xmlrpcValue = new Base64($value, true);
                break;
            case self::XMLRPC_TYPE_NIL:
                // Fall through to the next case
            case self::XMLRPC_TYPE_APACHENIL:
            ...
            case self::XMLRPC_TYPE_ARRAY:
                ...
                if (null === $data) {
                    require_once 'Zend/XmlRpc/Value/Exception.php';
                    throw new Exception('Invalid XML for XML-RPC native '. self::XMLRPC_TYPE_ARRAY .' type: ARRAY tag must contain DATA tag');
                }
                $values = array();
            ...
            case self::XMLRPC_TYPE_STRUCT:
                ...
                require_once 'Zend/XmlRpc/Value/Struct.php';
                $xmlrpcValue = new Struct($values);
                break;
            default:
                require_once 'Zend/XmlRpc/Value/Exception.php';
                throw new Exception('Value type \''. $type .'\' parsed from the XML string is not a known XML-RPC native type');
                break;
        }
    ...
    protected static function _createSimpleXMLElement(&$xml)
    {
        ...
        try {
            ...
            require_once 'Zend/XmlRpc/Value/Exception.php';
            throw new Exception('Failed to create XML-RPC value from XML string: ' . $e->getMessage(), $e->getCode(), $e);
        }
    }
