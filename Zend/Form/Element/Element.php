 ...
 */
class Element implements Validate\ValidateInterface
{
    /**
     * Element Constants
     */
    ...
    public function __construct($spec, $options = null)
    {
        ...
        } elseif (is_string($spec) && ($options instanceof Config\Config)) {
            $this->setConfig($options);
        }

        if (null === $this->getName()) {
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception('Zend_Form_Element requires each element to have a name');
        }
    ...
    public function setTranslator($translator = null)
    {
        ...
        } elseif ($translator instanceof \Zend\Translate\Adapter\Adapter) {
            $this->_translator = $translator;
        } elseif ($translator instanceof \Zend\Translate\Translate) {
            $this->_translator = $translator->getAdapter();
        } else {
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception('Invalid translator specified');
        }
        return $this;
    }
    ...
    public function getTranslator()
    {
        ...
        if (null === $this->_translator) {
            return Form\Form::getDefaultTranslator();
        }
    ...
    public function setName($name)
    {
        ...
        if ('' === $name) {
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception('Invalid name provided; must contain only valid variable characters and be non-empty');
        }
    ...
    public function setAttrib($name, $value)
    {
        ...
        if ('_' == $name[0]) {
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception(sprintf('Invalid attribute "%s"; must not contain a leading underscore', $name));
        }
    ...
    public function __get($key)
    {
        if ('_' == $key[0]) {
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception(sprintf('Cannot retrieve value for protected/private property "%s"', $key));
        }
    ...
    public function setPluginLoader(\Zend\Loader\PluginLoader\PluginLoaderInterface $loader, $type)
    {
        ...
        switch ($type) {
            ...
            default:
                require_once 'Zend/Form/Exception.php';
                throw new Form\Exception(sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
    }
    ...
    public function getPluginLoader($type)
    {
        ...
        switch ($type) {
            ...
            default:
                require_once 'Zend/Form/Exception.php';
                throw new Form\Exception(sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
        }
    }
    ...
    {
        if ($validator instanceof Validate\ValidateInterface) {
            ...
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception('Invalid validator provided to addValidator; must be string or Zend_Validate_Interface');
        }
    ...
    public function addValidators(array $validators)
    {
        foreach ($validators as $validatorInfo) {
            if (is_string($validatorInfo)) {
            ...
            } elseif ($validatorInfo instanceof Validate\ValidateInterface) {
                $this->addValidator($validatorInfo);
                ...
                require_once 'Zend/Form/Exception.php';
                throw new Form\Exception('Invalid validator passed to addValidators()');
            }
        }
    ...
    public function getValidators()
    {
        ...
        foreach ($this->_validators as $key => $value) {
            if ($value instanceof Validate\ValidateInterface) {
    ...
    public function isValid($value, $context = null)
    {
        ...
        if (Validate\AbstractValidate::hasDefaultTranslator() &&
            !Form\Form::hasDefaultTranslator())
        {
            $translator = Validate\AbstractValidate::getDefaultTranslator();
    ...
    public function addFilter($filter, $options = array())
    {
        if ($filter instanceof Filter\FilterInterface) {
            ...
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception('Invalid filter provided to addFilter; must be string or Zend_Filter_Interface');
        }
    ...
    public function addFilters(array $filters)
    {
        foreach ($filters as $filterInfo) {
            if (is_string($filterInfo)) {
            ...
            } elseif ($filterInfo instanceof Filter\FilterInterface) {
                $this->addFilter($filterInfo);
                ...
                require_once 'Zend/Form/Exception.php';
                throw new Form\Exception('Invalid filter passed to addFilters()');
            }
        }
    ...
    public function getFilters()
    {
        ...
        foreach ($this->_filters as $key => $value) {
            if ($value instanceof Filter\FilterInterface) {
                $filters[$key] = $value;
                continue;
            }
    ...
    public function addDecorator($decorator, $options = null)
    {
        if ($decorator instanceof Decorator\DecoratorInterface) {
            ...
            if (is_numeric($name)) {
                require_once 'Zend/Form/Exception.php';
                throw new Form\Exception('Invalid alias provided to addDecorator; must be alphanumeric string');
            }
            if (is_string($spec)) {
                $decorator = array(
                    'decorator' => $spec,
            ...
            } elseif ($spec instanceof Decorator\DecoratorInterface) {
                $decorator = $spec;
            }
        } else {
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception('Invalid decorator provided to addDecorator; must be string or Zend_Form_Decorator_Interface');
        }
    ...
    public function addDecorators(array $decorators)
    {
        foreach ($decorators as $decoratorName => $decoratorInfo) {
            if (is_string($decoratorInfo) ||
                $decoratorInfo instanceof Decorator\DecoratorInterface) {
                ...
                require_once 'Zend/Form/Exception.php';
                throw new Form\Exception('Invalid decorator passed to addDecorators()');
            }
        }
    ...
    protected function _loadFilter(array $filter)
    {
        ...
        if (array_key_exists($name, $this->_filters)) {
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception(sprintf('Filter instance already exists for filter "%s"', $origName));
        }
    ...
    protected function _loadValidator(array $validator)
    {
        ...
        if (array_key_exists($name, $this->_validators)) {
            require_once 'Zend/Form/Exception.php';
            throw new Form\Exception(sprintf('Validator instance already exists for validator "%s"', $origName));
        }
