...
class Date extends DateObject
{
    ...
    public function __construct($date = null, $part = null, $locale = null)
    {
        if (is_object($date) and !($date instanceof TimeSync\Protocol) and
            ...
            if ($locale instanceof Locale\Locale) {
                $locale = $date;
                $date   = null;
                $part   = null;
            } else {
        ...
        if (($date !== null) and !is_array($date) and !($date instanceof TimeSync\Protocol) and
            !($date instanceof Date) and !defined($date) and Locale\Locale::isLocale($date, true, false)) {
            $locale = $date;
            $date   = null;
            $part   = null;
        } else if (($part !== null) and !defined($part) and Locale\Locale::isLocale($part, true, false)) {
            $locale = $part;
            $part   = null;
        }
                ...
                switch($name) {
                    ...
                    case 'cache' :
                        if ($value === null) {
                            ...
                            parent::$_cache = $value;
                            parent::$_cacheTags = DateObject::_getTagSupportForCache();
    ...
    {
        if (is_object($format)) {
            if ($format instanceof Locale\Locale) {
                $locale = $format;
                $format = null;
            } else {
        ...
        if (is_object($type)) {
            if ($type instanceof Locale\Locale) {
                $locale = $type;
                $type   = null;
            } else {
                $type = (string) $type;
        ...
        if (($format !== null) && !defined($format)
            ...
            && Locale\Locale::isLocale($format, null, false)) {
            $locale = $format;
            $format = null;
        }

        if (($type !== null) and ($type != 'php') and ($type != 'iso') and
            Locale\Locale::isLocale($type, null, false)) {
            $locale = $type;
            $type = null;
        }
        ...
        if ($format === null) {
            $format = Locale\Format::getDateFormat($locale) . ' ' . Locale\Format::getTimeFormat($locale);
        } else if (((self::$_options['format_type'] == 'php') && ($type === null)) or ($type == 'php')) {
            $format = Locale\Format::convertPhpToIsoFormat($format);
        }
    ...
    public function get($part = null, $locale = null)
    {
        ...
        if (($part !== null) && !defined($part)
            ...
            && Locale\Locale::isLocale($part, null, false)) {
            $locale = $part;
            $part = null;
        }
        ...
        if ($part === null) {
            ...
            $part = Locale\Format::convertPhpToIsoFormat($part);
        }
    ...
    private function _parseIsoToDate($token, $locale) {
        switch($token) {
            ...
            case self::WEEKDAY_SHORT :
                ...
                $day     = Data\Data::getContent($locale, 'day', array('gregorian', 'format', 'wide', $weekday));
            ...
            case self::WEEKDAY :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                return $this->_toComment(Data\Data::getContent($locale, 'day', array('gregorian', 'format', 'wide', $weekday)));
                break;
            ...
            case 'EEEEE' :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                $day = Data\Data::getContent($locale, 'day', array('gregorian', 'format', 'abbreviated', $weekday));
                return $this->_toComment(iconv_substr($day, 0, 1, 'UTF-8'));
                break;

            case self::WEEKDAY_NAME :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                return $this->_toComment(Data\Data::getContent($locale, 'day', array('gregorian', 'format', 'abbreviated', $weekday)));
                break;
            ...
            case self::MONTH_NAME :
                ...
                return $this->_toComment(Data\Data::getContent($locale, 'month', array('gregorian', 'format', 'wide', $month)));
                break;
            ...
            case self::MONTH_NAME_SHORT :
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                return $this->_toComment(Data\Data::getContent($locale, 'month', array('gregorian', 'format', 'abbreviated', $month)));
                break;
            ...
            case self::MONTH_NAME_NARROW :
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                $mon = Data\Data::getContent($locale, 'month', array('gregorian', 'format', 'abbreviated', $month));
            ...
            case self::MERIDIEM :
                ...
                if ($am == 'am') {
                    return $this->_toComment(Data\Data::getContent($locale, 'am'));
                }

                return $this->_toComment(Data\Data::getContent($locale, 'pm'));
                break;
            ...
            case 'GGG' :
                ...
                if ($year < 0) {
                    return $this->_toComment(Data\Data::getContent($locale, 'era', array('gregorian', 'Abbr', '0')));
                }

                return $this->_toComment(Data\Data::getContent($locale, 'era', array('gregorian', 'Abbr', '1')));
                break;

            case self::ERA_NARROW :
                $year = $this->date('Y', $this->getUnixTimestamp(), false);
                if ($year < 0) {
                    return $this->_toComment(iconv_substr(Data\Data::getContent($locale, 'era', array('gregorian', 'Abbr', '0')), 0, 1, 'UTF-8')) . '.';
                }

                return $this->_toComment(iconv_substr(Data\Data::getContent($locale, 'era', array('gregorian', 'Abbr', '1')), 0, 1, 'UTF-8')) . '.';
                break;

            case self::ERA_NAME :
                $year = $this->date('Y', $this->getUnixTimestamp(), false);
                if ($year < 0) {
                    return $this->_toComment(Data\Data::getContent($locale, 'era', array('gregorian', 'Names', '0')));
                }

                return $this->_toComment(Data\Data::getContent($locale, 'era', array('gregorian', 'Names', '1')));
                break;

            case self::DATES :
                return $this->_toToken(Locale\Format::getDateFormat($locale), $locale);
                break;

            case self::DATE_FULL :
                return $this->_toToken(Data\Data::getContent($locale, 'date', array('gregorian', 'full')), $locale);
                break;

            case self::DATE_LONG :
                return $this->_toToken(Data\Data::getContent($locale, 'date', array('gregorian', 'long')), $locale);
                break;

            case self::DATE_MEDIUM :
                return $this->_toToken(Data\Data::getContent($locale, 'date', array('gregorian', 'medium')), $locale);
                break;

            case self::DATE_SHORT :
                return $this->_toToken(Data\Data::getContent($locale, 'date', array('gregorian', 'short')), $locale);
                break;

            case self::TIMES :
                return $this->_toToken(Locale\Format::getTimeFormat($locale), $locale);
                break;

            case self::TIME_FULL :
                return $this->_toToken(Data\Data::getContent($locale, 'time', 'full'), $locale);
                break;

            case self::TIME_LONG :
                return $this->_toToken(Data\Data::getContent($locale, 'time', 'long'), $locale);
                break;

            case self::TIME_MEDIUM :
                return $this->_toToken(Data\Data::getContent($locale, 'time', 'medium'), $locale);
                break;

            case self::TIME_SHORT :
                return $this->_toToken(Data\Data::getContent($locale, 'time', 'short'), $locale);
                break;

            case self::DATETIME :
                return $this->_toToken(Locale\Format::getDateTimeFormat($locale), $locale);
                break;

            case self::DATETIME_FULL :
                return $this->_toToken(Data\Data::getContent($locale, 'datetime', array('gregorian', 'full')), $locale);
                break;

            case self::DATETIME_LONG :
                return $this->_toToken(Data\Data::getContent($locale, 'datetime', array('gregorian', 'long')), $locale);
                break;

            case self::DATETIME_MEDIUM :
                return $this->_toToken(Data\Data::getContent($locale, 'datetime', array('gregorian', 'medium')), $locale);
                break;

            case self::DATETIME_SHORT :
                return $this->_toToken(Data\Data::getContent($locale, 'datetime', array('gregorian', 'short')), $locale);
                break;
    ...
    private function _assign($calc, $date, $comp = 0, $dst = false)
    {
        switch ($calc) {
            case 'set' :
                if (!empty($comp)) {
                    $this->setUnixTimestamp(call_user_func(Math\Math::$sub, $this->getUnixTimestamp(), $comp));
                }
                $this->setUnixTimestamp(call_user_func(Math\Math::$add, $this->getUnixTimestamp(), $date));
            ...
            case 'add' :
                ...
                break;
            case 'sub' :
                $this->setUnixTimestamp(call_user_func(Math\Math::$sub, $this->getUnixTimestamp(), $date));
                $value = $this->getUnixTimestamp();
                break;
            default :
                // cmp - compare
                return call_user_func(Math\Math::$comp, $comp, $date);
                break;
        }
    ...
    private function _calculate($calc, $date, $part, $locale)
    {
        ...
        if (($part !== null) && (strlen($part) !== 2) && (Locale\Locale::isLocale($part, null, false))) {
            $locale = $part;
            $part   = null;
        }
        ...
        switch($part) {
            ...
            case self::WEEKDAY_SHORT:
                $daylist = Data\Data::getList($locale, 'day');
            ...
            case self::WEEKDAY_NARROW:
                $daylist = Data\Data::getList($locale, 'day', array('gregorian', 'format', 'abbreviated'));
                $weekday = (int) $this->toString(self::WEEKDAY_DIGIT, 'iso', $locale);
                $cnt = 0;
                foreach ($daylist as $key => $value) {
            ...
            // month formats
            case self::MONTH_NAME:
                $monthlist = Data\Data::getList($locale, 'month');
                $cnt = 0;
            ...
            case self::MONTH_NAME_SHORT:
                $monthlist = Data\Data::getList($locale, 'month', array('gregorian', 'format', 'abbreviated'));
                $cnt = 0;
                foreach ($monthlist as $key => $value) {
                    if (strtoupper($value) == strtoupper($date)) {
                        $found = $key;
            ...
            case self::MONTH_NAME_NARROW:
                $monthlist = Data\Data::getList($locale, 'month', array('gregorian', 'stand-alone', 'narrow'));
                $cnt       = 0;
                foreach ($monthlist as $key => $value) {
                    if (strtoupper($value) === strtoupper($date)) {
                        $found = $key;
            ...
            case self::DATES:
                try {
                    $parsed = Locale\Format::getDate($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
            ...
            case self::DATE_FULL:
                try {
                    $format = Data\Data::getContent($locale, 'date', array('gregorian', 'full'));
                    $parsed = Locale\Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_LONG:
                try {
                    $format = Data\Data::getContent($locale, 'date', array('gregorian', 'long'));
                    $parsed = Locale\Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_MEDIUM:
                try {
                    $format = Data\Data::getContent($locale, 'date', array('gregorian', 'medium'));
                    $parsed = Locale\Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_SHORT:
                try {
                    $format = Data\Data::getContent($locale, 'date', array('gregorian', 'short'));
                    $parsed = Locale\Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::TIMES:
                try {
                    ...
                    $parsed = Locale\Format::getTime($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
            ...
            case self::TIME_FULL:
                try {
                    $format = Data\Data::getContent($locale, 'time', array('gregorian', 'full'));
                    $parsed = Locale\Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
                        $year  = 1970;
            ...
            case self::TIME_LONG:
                try {
                    $format = Data\Data::getContent($locale, 'time', array('gregorian', 'long'));
                    $parsed = Locale\Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::TIME_MEDIUM:
                try {
                    $format = Data\Data::getContent($locale, 'time', array('gregorian', 'medium'));
                    $parsed = Locale\Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::TIME_SHORT:
                try {
                    $format = Data\Data::getContent($locale, 'time', array('gregorian', 'short'));
                    $parsed = Locale\Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::DATETIME:
                try {
                    $parsed = Locale\Format::getDateTime($date, array('locale' => $locale, 'format_type' => 'iso', 'fix_date' => true));
                    if (($calc == 'set') || ($calc == 'cmp')) {
                        --$parsed['month'];
                        --$month;
                        --$parsed['day'];
            ...
            case self::DATETIME_FULL:
                try {
                    $format = Data\Data::getContent($locale, 'datetime', array('gregorian', 'full'));
                    $parsed = Locale\Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATETIME_LONG:
                try {
                    $format = Data\Data::getContent($locale, 'datetime', array('gregorian', 'long'));
                    $parsed = Locale\Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATETIME_MEDIUM:
                try {
                    $format = Data\Data::getContent($locale, 'datetime', array('gregorian', 'medium'));
                    $parsed = Locale\Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if (($calc == 'set') || ($calc == 'cmp')) {
                        --$parsed['month'];
                        --$month;
            ...
            case self::DATETIME_SHORT:
                try {
                    $format = Data\Data::getContent($locale, 'datetime', array('gregorian', 'short'));
                    $parsed = Locale\Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            default:
                if (!is_numeric($date) || !empty($part)) {
                    try {
                        if (empty($part)) {
                            $part  = Locale\Format::getDateFormat($locale) . " ";
                            $part .= Locale\Format::getTimeFormat($locale);
                        }

                        $parsed = Locale\Format::getDate($date, array('date_format' => $part, 'locale' => $locale, 'fix_date' => true, 'format_type' => 'iso'));
        ...
        if ($time instanceof Date) {
            ...
            if (is_array($time)) {
                ...
                try {
                    ...
                    $parsed = Locale\Format::getTime($time, array('date_format' => $format, 'locale' => $locale, 'format_type' => 'iso'));
                } catch (Locale\Exception $e) {
                    require_once 'Zend/Date/Exception.php';
        ...
        if ($date instanceof Date) {
            ...
            if (is_array($date)) {
                ...
                try {
                    ...
                    $parsed = Locale\Format::getDate($date, array('date_format' => $format, 'locale' => $locale, 'format_type' => 'iso'));
    ...
    private function _month($calc, $month, $locale)
    {
        ...
        if ($month instanceof Date) {
            ...
            if (is_numeric($month)) {
                ...
                $monthlist  = Data\Data::getList($locale, 'month');
                $monthlist2 = Data\Data::getList($locale, 'month', array('gregorian', 'format', 'abbreviated'));
    ...
    public static function isDate($date, $format = null, $locale = null)
    {
        ...
        if ($format === null) {
            $format = Locale\Format::getDateFormat($locale);
        ...
        if (!is_array($date)) {
            try {
                $parsed = Locale\Format::getDate($date, array('locale' => $locale,
    ...
    protected static function _getLocalizedToken($token, $locale)
    {
        switch($token) {
            ...
            case self::RFC_2822 :
                ...
                break;
            case self::DATES :
                return Data\Data::getContent($locale, 'date');
                break;
            case self::DATE_FULL :
                return Data\Data::getContent($locale, 'date', array('gregorian', 'full'));
                break;
            case self::DATE_LONG :
                return Data\Data::getContent($locale, 'date', array('gregorian', 'long'));
                break;
            case self::DATE_MEDIUM :
                return Data\Data::getContent($locale, 'date', array('gregorian', 'medium'));
                break;
            case self::DATE_SHORT :
                return Data\Data::getContent($locale, 'date', array('gregorian', 'short'));
                break;
            case self::TIMES :
                return Data\Data::getContent($locale, 'time');
                break;
            case self::TIME_FULL :
                return Data\Data::getContent($locale, 'time', array('gregorian', 'full'));
                break;
            case self::TIME_LONG :
                return Data\Data::getContent($locale, 'time', array('gregorian', 'long'));
                break;
            case self::TIME_MEDIUM :
                return Data\Data::getContent($locale, 'time', array('gregorian', 'medium'));
                break;
            case self::TIME_SHORT :
                return Data\Data::getContent($locale, 'time', array('gregorian', 'short'));
                break;
            case self::DATETIME :
                return Data\Data::getContent($locale, 'datetime');
                break;
            case self::DATETIME_FULL :
                return Data\Data::getContent($locale, 'datetime', array('gregorian', 'full'));
                break;
            case self::DATETIME_LONG :
                return Data\Data::getContent($locale, 'datetime', array('gregorian', 'long'));
                break;
            case self::DATETIME_MEDIUM :
                return Data\Data::getContent($locale, 'datetime', array('gregorian', 'medium'));
                break;
            case self::DATETIME_SHORT :
                return Data\Data::getContent($locale, 'datetime', array('gregorian', 'short'));
                break;
            case self::ATOM :
            case self::RFC_3339 :
            case self::W3C :
