 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Db extends Queue\Adapter\AdapterAbstract
{
    /**
    ...
    public function __construct($options, Queue\Queue $queue = null)
    {
        ...
        if (!is_bool($this->_options['options'][Select\Select::FOR_UPDATE])) {
            ...
            throw new Queue\Exception('Options array item: Zend_Db_Select::FOR_UPDATE must be boolean');
        }
    ...
    protected function _initDbAdapter()
    {
        ...
        try {
            $db = \Zend\Db\Db::factory($type, $options);
    ...
    public function count(Queue\Queue $queue = null)
    {
        ...
        $query = $db->select();
        $query->from($info['name'], array(new \Zend\Db\Expr('COUNT(1)')))
    ...
    * Messsage management functions
     ...
     * @throws Zend_Queue_Exception - database error
     */
    public function deleteMessage(Queue\Message\Message $message)
    {
        $db    = $this->_messageTable->getAdapter();
