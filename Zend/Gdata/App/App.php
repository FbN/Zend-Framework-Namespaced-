...
class App
{
     ...
     * @return void
     */
    public static function setStaticHttpClient(Client\Client $httpClient)
    {
    ...
    {
        if (!self::$_staticHttpClient instanceof Client\Client) {
            $client = new Client\Client();
            $userAgent = 'Zend_Framework_Gdata/' . end\Version::VERSION;
            $client->setHeaders('User-Agent', $userAgent);
            $client->setConfig(array(
                'strictredirects' => true
                )
                                   ...
                                   $contentTypeOverride = null)
    {
        ...
        if ($method != 'POST' && $method != 'GET' && App::getHttpMethodOverride()) {
        ...
        $body = null, $contentType = null, $remainingRedirects = null)
    {
        ...
        $uri = end\Uri\Http::fromString($url);
        ...
        } catch (end\Http\Client\Exception $e) {
            // reset adapter
            if ($usingMimeStream) {
                $this->_httpClient->setAdapter($oldHttpAdapter);
            }
            ...
            foreach ($this->_registeredPackages as $name) {
                 try {
                     ...
                     if (!class_exists($name . '_' . $class, false)) {
                        require_once 'Zend/Loader.php';
                        @Loader\Loader::loadClass($name . '_' . $class);
                     }
