...
use Zend\Gdata\App;
use Zend\Gdata\Media\Extension;
...
class VideoEntry extends MediaEntry
{
    ...
    protected function takeChildFromDOM($child)
    {
        ...
        case $this->lookupNamespace('gd') . ':' . 'rating':
            $rating = new Extension\Rating();
            $rating->transferFromDOM($child);
            $this->_rating = $rating;
            break;
        case $this->lookupNamespace('gd') . ':' . 'comments':
            $comments = new Extension\Comments();
        ...
        case $this->lookupNamespace('gd') . ':' . 'feedLink':
            $feedLink = new Extension\FeedLink();
    ...
    {
        $this->setRecorded(
            new Extension\Recorded($recorded));
        return $this;
    }
    ...
    public function setVideoTitle($title)
    {
        ...
        $this->getMediaGroup()->setTitle(
            new Extension\MediaTitle($title));
        return $this;
    }
    ...
    public function setVideoDescription($description)
    {
        ...
        $this->getMediaGroup()->setDescription(
            new Extension\MediaDescription($description));
        return $this;
    }
    ...
    {
        $this->ensureMediaGroupIsNotNull();
        $this->getMediaGroup()->setPrivate(new Extension\Private());
        return $this;
    }
    ...
    {
        $this->ensureMediaGroupIsNotNull();
        $keywords = new Extension\MediaKeywords();
        if (is_array($tags)) {
            $tags = implode(', ', $tags);
        }
    ...
    {
        $this->ensureMediaGroupIsNotNull();
        $this->getMediaGroup()->setCategory(array(new Extension\MediaCategory($category, self::YOUTUBE_CATEGORY_SCHEMA)));
        return $this;
    }
    ...
    public function getVideoDeveloperTags()
    {
        ...
        if ($categoryArray != null) {
            foreach ($categoryArray as $category) {
                if ($category instanceof Extension\MediaCategory) {
    ...
    {
        $this->ensureMediaGroupIsNotNull();
        $newCategory = new Extension\MediaCategory($developerTag, self::YOUTUBE_DEVELOPER_TAGS_SCHEMA);
    ...
    public function setVideoRating($ratingValue)
    {
        if ($ratingValue < 1 || $ratingValue > 5) {
         ...
         require_once 'Zend/Gdata/Extension/Rating.php';
         $rating = new Extension\Rating(null, 1, 5, null,
            $ratingValue);
