...
class Request extends Token
{
    ...
    public function __construct(
        \Zend\Http\Response\Response $response = null,
        \Zend\Oauth\Http\Utility $utility = null
    ) {
        parent::__construct($response, $utility);

        // detect if server supports OAuth 1.0a
        if (isset($this->_params[Token::TOKEN_PARAM_CALLBACK_CONFIRMED])) {
            \Zend\Oauth\Client::$supportsRevisionA = true;
        }
    }
}
