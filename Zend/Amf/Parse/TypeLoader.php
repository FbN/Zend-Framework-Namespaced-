...
final class TypeLoader
{
    ...
    public static function handleResource($resource)
    {
        if(!self::$_resourceLoader) {
            ...
            throw new Amf\Exception('Unable to handle resources - resource plugin loader not set');
        }
        try {
            while(is_resource($resource)) {
                ...
                if(!$resclass) {
                    require_once 'Zend/Amf/Exception.php';
                    throw new Amf\Exception('Can not serialize resource type: '. get_resource_type($resource));
                }
                $parser = new $resclass();
                if(\is\callable(array($parser, 'parse'))) {
                    $resource = $parser->parse($resource);
                } else {
                    require_once 'Zend/Amf/Exception.php';
                    throw new Amf\Exception("Could not call parse() method on class $resclass");
                }
            }
            return $resource;
        } catch(Amf\Exception $e) {
            throw new Amf\Exception($e->getMessage(), $e->getCode(), $e);
        } catch(\Exception $e) {
            require_once 'Zend/Amf/Exception.php';
            throw new Amf\Exception('Can not serialize resource type: '. get_resource_type($resource), 0, $e);
        }
    }
}
