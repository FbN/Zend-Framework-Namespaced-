...
class BinaryStream
{
    ...
    public function __construct($stream)
    {
        if (!is_string($stream)) {
            ...
            throw new Amf\Exception('Inputdata is not of type String');
        }
    ...
    public function readBytes($length)
    {
        if (($length + $this->_needle) > $this->_streamLength) {
            require_once 'Zend/Amf/Exception.php';
            throw new Amf\Exception('Buffer underrun at needle position: ' . $this->_needle . ' while requesting length: ' . $length);
        }
