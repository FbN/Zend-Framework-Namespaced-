 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Auth extends \Zend\Amf\Auth\AbstractAuth
{
...
*/
        foreach($xml->role as $role) {
            $this->_acl->addRole(new \Zend\Acl\Role\Role((string)$role["id"]));
    ...
    public function authenticate()
    {
        if (empty($this->_username) ||
            ...
            throw new Auth\Adapter\Exception('Username/password should be set');
        }

        if(!isset($this->_users[$this->_username])) {
            return new Auth\Result(Auth\Result::FAILURE_IDENTITY_NOT_FOUND,
                null,
        ...
        if($user["password"] != $this->_password) {
            return new Auth\Result(Auth\Result::FAILURE_CREDENTIAL_INVALID,
                null,
        ...
        return new Auth\Result(Auth\Result::SUCCESS, $id);
    }
}
