...
class Server implements \Zend\Server\ServerInterface
{
    ...
    {
        Parse\TypeLoader::setResourceLoader(new PluginLoader\PluginLoader(array("Zend_Amf_Parse_Resource" => "Zend/Amf/Parse/Resource")));
    }
     ...
     */
    public function setAuth(Amf\Auth\AbstractAuth $auth)
    {
        $this->_auth = $auth;
        return $this;
    }
   ...
   /**
    ...
    public function setSession($namespace = 'Zend_Amf')
    {
        ...
        $this->_session = true;
        $this->_sesionNamespace = new \Zend\Session\Namespace($namespace);
        return $this;
    }
    ...
    protected function _checkAcl($object, $function)
    {
        ...
        if($object) {
            ...
            if(!$this->_acl->has($class)) {
                ...
                $this->_acl->add(new \Zend\Acl\Resource\Resource($class));
            }
        ...
        if($auth->hasIdentity()) {
            ...
            if($this->_acl->hasRole(Amf\Constants::GUEST_ROLE)) {
                $role = Amf\Constants::GUEST_ROLE;
            } else {
    ...
    protected function getLoader()
    {
        if(empty($this->_loader)) {
            require_once 'Zend/Loader/PluginLoader.php';
            $this->_loader = new PluginLoader\PluginLoader();
        }
        return $this->_loader;
    }
    ...
    {
        if($source) {
            if(($mapped = Parse\TypeLoader::getMappedClassName($source)) !== false) {
                $source = $mapped;
            }
        }
        ...
        if ($info instanceof Reflection\Function\Function) {
            $func = $info->getName();
            $this->_checkAcl(null, $func);
            $return = call_user_func_array($func, $params);
        } elseif ($info instanceof Reflection\Method) {
            // Get class
     ...
     */
    protected function _loadCommandMessage(Messaging\CommandMessage $message)
    {
        require_once 'Zend/Amf/Value/Messaging/AcknowledgeMessage.php';
        switch($message->operation) {
            case Messaging\CommandMessage::DISCONNECT_OPERATION :
            case Messaging\CommandMessage::CLIENT_PING_OPERATION :
                $return = new Messaging\AcknowledgeMessage($message);
                break;
            case Messaging\CommandMessage::LOGIN_OPERATION :
                ...
                $return = new Messaging\AcknowledgeMessage($message);
                break;
           case Messaging\CommandMessage::LOGOUT_OPERATION :
                if($this->_auth) {
                    Auth\Auth::getInstance()->clearIdentity();
                }
                $return = new Messaging\AcknowledgeMessage($message);
                break;
            default :
    ...
    protected function _errorMessage($objectEncoding, $message, $description, $detail, $code, $line)
    {
        ...
        switch ($objectEncoding) {
            case Amf\Constants::AMF0_OBJECT_ENCODING :
                return array (
            ...
            case Amf\Constants::AMF3_OBJECT_ENCODING :
     ...
     */
    protected function _handle(Request\Request $request)
    {
        ...
        if ($this->_auth) {
            ...
            if (isset($headers[Amf\Constants::CREDENTIALS_HEADER]) &&
                isset($headers[Amf\Constants::CREDENTIALS_HEADER]->userid)) {
                $handleAuth = true;
            }
        }
        ...
        foreach($responseBody as $body)
        {
            try {
                if ($handleAuth) {
                    if ($this->_handleAuth(
                        $headers[Amf\Constants::CREDENTIALS_HEADER]->userid,
                        $headers[Amf\Constants::CREDENTIALS_HEADER]->password)) {
                        // use RequestPersistentHeader to clear credentials
                        $response->addAmfHeader(
                            new Value\MessageHeader(
                                Amf\Constants::PERSISTENT_HEADER,
                                false,
                                new Value\MessageHeader(
                                    Amf\Constants::CREDENTIALS_HEADER,
                                    false, null)));
                        $handleAuth = false;
                    }
                }

                if ($objectEncoding == Amf\Constants::AMF0_OBJECT_ENCODING) {
                    ...
                    if ($message instanceof Messaging\CommandMessage) {
        ...
        if($this->isSession()) {
           ...
           if(isset($_SERVER['QUERY_STRING'])) {
            ...
            $sessionHeader = new Value\MessageHeader(Amf\Constants::URL_APPEND_HEADER, false, $sessionValue);
    ...
    public function handle($request = null)
    {
        ...
        if ($request === null || !$request instanceof Request\Request) {
    ...
    public function setRequest($request)
    {
        ...
        } elseif (!$request instanceof Request\Request) {
            require_once 'Zend/Amf/Server/Exception.php';
    ...
    public function setResponse($response)
    {
        ...
        } elseif (!$response instanceof Response\Response) {
            require_once 'Zend/Amf/Server/Exception.php';
    ...
    protected function _buildDispatchTable()
    {
        ...
        foreach ($this->_methods as $key => $dispatchable) {
            if ($dispatchable instanceof Reflection\Function\AbstractFunction) {
            ...
            if ($dispatchable instanceof Reflection\Class) {
    ...
    {
        require_once 'Zend/Amf/Parse/TypeLoader.php';
        Parse\TypeLoader::setMapping($asClass, $phpClass);
        return $this;
    }
