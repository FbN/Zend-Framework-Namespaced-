...
abstract class AbstractAdapter
{
    ...
    public function addValidator($validator, $breakChainOnFailure = false, $options = null, $files = null)
    {
        if ($validator instanceof Validate\ValidateInterface) {
            ...
            if (\is\array($options) && isset($options['messages'])) {
            ...
            require_once 'Zend/File/Transfer/Exception.php';
            throw new Transfer\Exception('Invalid validator provided to addValidator; must be string or Zend_Validate_Interface');
        }
    ...
    public function addValidators(array $validators, $files = null)
    {
        foreach ($validators as $name => $validatorInfo) {
            if ($validatorInfo instanceof Validate\ValidateInterface) {
