...
class Server implements \Zend\Server\ServerInterface
{
    ...
    public function __construct()
    {
        ...
        $this->_reflection = new Reflection\Reflection();
    }
    ...
    public function handle($request = false)
    {
        ...
        if (isset($request['method'])) {
            ...
            if (isset($this->_functions[$this->_method])) {
                if ($this->_functions[$this->_method] instanceof Reflection\Function\Function || $this->_functions[$this->_method] instanceof Reflection\Method && $this->_functions[$this->_method]->isPublic()) {
                    ...
                    if (count($calling_args) < count($func_args)) {
                        ...
                        $result = $this->fault(new Exception('Invalid Method Call to ' . $this->_method . '. Missing argument(s): ' . implode(', ', $missing_args) . '.'), 400);
                    }

                    if (!$result && $this->_functions[$this->_method] instanceof Reflection\Method) {
                        // Get class
                    ...
                    require_once "Zend/Rest/Server/Exception.php";
                    $result = $this->fault(
                        new Exception("Unknown Method '$this->_method'."),
                        404
                    );
                }
            } else {
            ...
            require_once "Zend/Rest/Server/Exception.php";
            $result = $this->fault(
                new Exception("No Method Specified."),
                404
            );
        }
    ...
    protected function _handleStruct($struct)
    {
        ...
        if ($function instanceof Reflection\Method) {
    ...
    public function fault($exception = null, $code = null)
    {
        ...
        if ($function instanceof Reflection\Function\AbstractFunction) {
            $method = $function->getName();
        } else {
            $method = $function;
        }
    ...
    protected function _callObjectMethod($class, array $args)
    {
        try {
            ...
            require_once 'Zend/Rest/Server/Exception.php';
            throw new Exception('Error instantiating class ' . $class .
