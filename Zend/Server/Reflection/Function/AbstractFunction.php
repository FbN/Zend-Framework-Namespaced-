 ...
 * @namespace
 */
namespace Zend\Server\Reflection\Function;
...
abstract class AbstractFunction
{
    ...
    public function __construct(\Reflector $r, $namespace = null, $argv = array())
    {
        ...
        if ((!$r instanceof \ReflectionFunction)
            ...
            throw new Reflection\Exception('Invalid reflection class');
        }
        $this->_reflection = $r;
     ...
     * @return void
     */
    protected function _addTree(Reflection\Node $parent, $level = 0)
    {
        ...
        foreach ($this->_sigParams[$level] as $value) {
            $node = new Reflection\Node($value, $parent);
    ...
    protected function _buildTree()
    {
        ...
        foreach ((array) $this->_return as $value) {
            $node = new Reflection\Node($value);
            $this->_addTree($node);
            $returnTree[] = $node;
        }
    ...
    protected function _buildSignatures($return, $returnDesc, $paramTypes, $paramDesc)
    {
        ...
        foreach ($endPoints as $node) {
            if (!$node instanceof Reflection\Node) {
                continue;
            }
            ...
            do {
                ...
                $node = $node->getParent();
            } while ($node instanceof Reflection\Node);
        ...
        foreach ($signatures as $signature) {
            $return = new Reflection\ReturnValue(array_shift($signature), $this->_returnDesc);
            $tmp    = array();
            foreach ($signature as $key => $type) {
                $param = new Reflection\Parameter($params[$key], $type, (isset($this->_paramDesc[$key]) ? $this->_paramDesc[$key] : null));
                $param->setPosition($key);
                $tmp[] = $param;
            }

            $this->_prototypes[] = new Reflection\Prototype($return, $tmp);
        }
    }
    ...
    public function __call($method, $args)
    {
        ...
        require_once 'Zend/Server/Reflection/Exception.php';
        throw new Reflection\Exception('Invalid reflection method ("' .$method. '")');
    }
    ...
    public function setNamespace($namespace)
    {
        ...
        if (!is_string($namespace) || !preg_match('/[a-z0-9_\.]+/i', $namespace)) {
            require_once 'Zend/Server/Reflection/Exception.php';
            throw new Reflection\Exception('Invalid namespace');
        }
    ...
    public function setDescription($string)
    {
        if (!is_string($string)) {
            require_once 'Zend/Server/Reflection/Exception.php';
            throw new Reflection\Exception('Invalid description');
        }
