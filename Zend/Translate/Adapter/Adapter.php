...
abstract class Adapter {
    ...
    public function addTranslation($options = array())
    {
        ...
        if ((array_key_exists('log', $options)) && !($options['log'] instanceof Log\Log)) {
            require_once 'Zend/Translate/Exception.php';
            throw new Translate\Exception('Instance of Zend_Log expected for option log');
        }

        try {
            if (!($options['content'] instanceof Translate\Translate) && !($options['content'] instanceof Adapter)) {
                if (empty($options['locale'])) {
                    $options['locale'] = null;
                }
            ...
            require_once 'Zend/Translate/Exception.php';
            throw new Translate\Exception("The given Language '{$options['locale']}' does not exist", 0, $e);
        }
    ...
    public function setOptions(array $options = array())
    {
        ...
        foreach ($options as $key => $option) {
            if ($key == 'locale') {
                ...
                if (($key == 'log') && !($option instanceof Log\Log)) {
                    require_once 'Zend/Translate/Exception.php';
                    throw new Translate\Exception('Instance of Zend_Log expected for option log');
                }
    ...
    public function setLocale($locale)
    {
        ...
        try {
            ...
            require_once 'Zend/Translate/Exception.php';
            throw new Translate\Exception("The given Language ({$locale}) does not exist", 0, $e);
        }
    ...
    public function translate($messageId, $locale = null)
    {
        ...
        if (!Locale\Locale::isLocale($locale, true, false)) {
            if (!Locale\Locale::isLocale($locale, false, false)) {
                ...
                $rule = Translate\Plural::getPlural($number, $plocale);
        ...
        if ((is_string($messageId) || is_int($messageId)) && isset($this->_translate[$locale][$messageId])) {
            ...
            $rule = Translate\Plural::getPlural($number, $locale);
     ...
     */
    public static function setCache(Cache\Core $cache)
    {
        self::$_cache = $cache;
