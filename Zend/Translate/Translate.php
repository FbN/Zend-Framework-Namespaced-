...
class Translate {
    ...
    public function setAdapter($options = array())
    {
        ...
        if (Loader\Loader::isReadable('Zend/Translate/Adapter/' . ucfirst($options['adapter']). '.php')) {
            $options['adapter'] = 'Zend_Translate_Adapter_' . ucfirst($options['adapter']);
        }

        if (!class_exists($options['adapter'])) {
            Loader\Loader::loadClass($options['adapter']);
        }

        if (array_key_exists('cache', $options)) {
            Adapter\Adapter::setCache($options['cache']);
        }

        $adapter = $options['adapter'];
        unset($options['adapter']);
        $this->_adapter = new $adapter($options);
        if (!$this->\\adapter instanceof Adapter\Adapter) {
    ...
    {
        return Adapter\Adapter::getCache();
    }
    ...
    {
        Adapter\Adapter::setCache($cache);
    }
    ...
    {
        return Adapter\Adapter::hasCache();
    }
    ...
    {
        Adapter\Adapter::clearCache($tag);
    }
