...
class Format
{
            ...
            switch($name) {
                ...
                case 'date_format' :
                    if ($value == Format::STANDARD) {
                        ...
                        $options['date_format'] = Format::getDateFormat($locale);
                    } else if ((gettype($value) !== 'string') and ($value !== NULL)) {
                        require_once 'Zend/Locale/Exception.php';
                        ...
                        if (((isset($options['format_type']) === true) and ($options['format_type'] == 'php')) or
                            ...
                            $options['date_format'] = Format::convertPhpToIsoFormat($value);
                        }
                    }
                    break;
    ...
    public static function isNumber($input, array $options = array())
    {
        ...
        $regexs = Format::_getRegexForType('decimalnumber', $options);
        $regexs = array_merge($regexs, Format::_getRegexForType('scientificnumber', $options));
    ...
    {
        $options['number_format'] = Format::STANDARD;
