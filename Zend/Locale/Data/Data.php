...
class Data
{
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            if (!file_exists($filename)) {
                ...
                throw new Locale\Exception("Missing locale file '$filename' for '$locale' locale.");
            }
    ...
    private static function _checkLocale($locale)
    {
        ...
        if (!(Locale\Locale::isLocale((string) $locale, null, false))) {
            require_once 'Zend/Locale/Exception.php';
            throw new Locale\Exception("Locale (" . (string) $locale . ") is a unknown locale");
        }
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        if (!isset(self::$_cache) && !self::$_cacheDisabled) {
            ...
            self::$_cache = Cache\Cache::factory(
                'Core',
                'File',
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'Zend/Locale/Exception.php';
                throw new Locale\Exception("Unknown list ($path) for parsing locale data.");
                break;
        }
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'Zend/Locale/Exception.php';
                throw new Locale\Exception("Unknown detail ($path) for parsing locale data.");
                break;
        }
    ...
    {
        if (self::$_cacheTags) {
            self::$_cache->clean(Cache\Cache::CLEANING_MODE_MATCHING_TAG, array('Zend_Locale'));
        } else {
