...
abstract class SegmentWriter
{
     ...
     * @return integer
     */
    public function addField(\Zend\Search\Lucene\Field $field)
    {
        if (!isset($this->_fields[$field->name])) {
            $fieldNumber = count($this->_fields);
            $this->_fields[$field->name] =
                                new Index\FieldInfo($field->name,
                                                                       $field->isIndexed,
                                                                       $fieldNumber,
    ...
    public function addFieldInfo(Index\FieldInfo $fieldInfo)
    {
        if (!isset($this->_fields[$fieldInfo->name])) {
            ...
            $this->_fields[$fieldInfo->name] =
                                new Index\FieldInfo($fieldInfo->name,
                                                                       $fieldInfo->isIndexed,
                                                                       $fieldNumber,
...
// not supported yet            0x08 /* term offsets are stored with the term vectors */   |
    ...
    public function addTerm($termEntry, $termDocs)
    {
        ...
        $term = new Index\Term($termEntry->text,
                                                  $this->_fields[$termEntry->field]->number);
        $termInfo = new Index\TermInfo(count($termDocs),
     ...
     */
    protected function _dumpTermDictEntry(\Zend\Search\Lucene\Storage\File\File $dicFile,
