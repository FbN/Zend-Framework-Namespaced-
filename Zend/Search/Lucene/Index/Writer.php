...
class Writer
{
     ...
     */
    public static function createIndex(Directory\Directory $directory, $generation, $nameCount)
    {
        if ($generation == 0) {
            ...
            $segmentsFile = $directory->createFile(Lucene\Lucene::getSegmentFileName($generation));
     ...
     */
    public function __construct(Directory\Directory $directory, &$segmentInfos, $targetFormatVersion)
    {
     ...
     */
    public function addDocument(Lucene\Document\Document $document)
    {
    ...
    private function _updateSegments()
    {
        ...
        $generation = Lucene\Lucene::getActualGeneration($this->_directory);
        $segmentsFile   = $this->_directory->getFileObject(Lucene\Lucene::getSegmentFileName($generation), false);
        $newSegmentFile = $this->_directory->createFile(Lucene\Lucene::getSegmentFileName(++$generation), false);
        ...
        try {
            // Write format marker
            if ($this->_targetFormatVersion == Lucene\Lucene::FORMAT_2_1) {
                $newSegmentFile->writeInt((int)0xFFFFFFFD);
            } else if ($this->_targetFormatVersion == Lucene\Lucene::FORMAT_2_3) {
            ...
            if ($format == (int)0xFFFFFFFF) {
                $srcFormat = Lucene\Lucene::FORMAT_PRE_2_1;
            } else if ($format == (int)0xFFFFFFFD) {
                $srcFormat = Lucene\Lucene::FORMAT_2_1;
            } else if ($format == (int)0xFFFFFFFC) {
                $srcFormat = Lucene\Lucene::FORMAT_2_3;
            } else {
            ...
            for ($count = 0; $count < $segmentsCount; $count++) {
                ...
                if ($srcFormat == Lucene\Lucene::FORMAT_PRE_2_1) {
                    ...
                    if ($srcFormat == Lucene\Lucene::FORMAT_2_3) {
                ...
                if (!in_array($segName, $this->_segmentsToDelete)) {
                    ...
                    if ($this->_targetFormatVersion == Lucene\Lucene::FORMAT_2_3) {
        ...
        if (Lucene\LockManager::escalateReadLock($this->_directory)) {
            ...
            foreach ($this->_directory->fileList() as $file) {
                if ($file == 'deletable') {
                    ...
                    if ($file != Lucene\Lucene::getSegmentFileName($generation)) {
                        $filesToDelete[] = $file;
