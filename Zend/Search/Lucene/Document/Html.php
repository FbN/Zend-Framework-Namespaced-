...
class Html extends Document
{
    ...
    private function __construct($data, $isFile, $storeContent, $defaultEncoding = '')
    {
        ...
        $this->addField(Lucene\Field::Text('title', $docTitle, 'UTF-8'));

        $metaNodes = $xpath->query('/html/head/meta[@name]');
        foreach ($metaNodes as $metaNode) {
            $this->addField(Lucene\Field::Text($metaNode->getAttribute('name'),
        ...
        if ($storeContent) {
            $this->addField(Lucene\Field::Text('body', $docBody, 'UTF-8'));
        } else {
            $this->addField(Lucene\Field::UnStored('body', $docBody, 'UTF-8'));
        }
    ...
    {
        return new Html($data, false, $storeContent, $defaultEncoding);
    }
    ...
    {
        return new Html($file, true, $storeContent, $defaultEncoding);
    }
