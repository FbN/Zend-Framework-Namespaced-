 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class OpenXml extends Document
{
    /**
    ...
    protected function extractMetaData(\ZipArchive $package)
    {
        ...
        foreach ($relations->Relationship as $rel) {
            if ($rel["Type"] == OpenXml::SCHEMA_COREPROPERTIES) {
                ...
                foreach ($contents->children(OpenXml::SCHEMA_DUBLINCORE) as $child) {
                    $coreProperties[$child->getName()] = (string)$child;
                }
                foreach ($contents->children(OpenXml::SCHEMA_COREPROPERTIES) as $child) {
                    $coreProperties[$child->getName()] = (string)$child;
                }
                foreach ($contents->children(OpenXml::SCHEMA_DUBLINCORETERMS) as $child) {
                    $coreProperties[$child->getName()] = (string)$child;
                }
            }
        }
