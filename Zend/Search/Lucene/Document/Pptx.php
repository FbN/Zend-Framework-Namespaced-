...
class Pptx extends OpenXml
{
    ...
    private function __construct($fileName, $storeContent)
    {
        ...
        foreach ($relations->Relationship as $rel) {
            if ($rel["Type"] == OpenXml::SCHEMA_OFFICEDOCUMENT) {
                ...
                foreach ($slideRelations->Relationship as $slideRel) {
                    if ($slideRel["Type"] == Pptx::SCHEMA_SLIDERELATION) {
                        // Found slide!
                        ...
                        foreach ($slideNotesRelations->Relationship as $slideNoteRel) {
                            if ($slideNoteRel["Type"] == Pptx::SCHEMA_SLIDENOTESRELATION) {
                                // Found slide notes!
        ...
        foreach ($slides as $slideKey => $slide) {
            // Register namespaces
            $slide->registerXPathNamespace("p", Pptx::SCHEMA_PRESENTATIONML);
            $slide->registerXPathNamespace("a", Pptx::SCHEMA_DRAWINGML);
            ...
            if (isset($slideNotes[$slideKey])) {
                ...
                // Register namespaces
                $slideNote->registerXPathNamespace("p", Pptx::SCHEMA_PRESENTATIONML);
                $slideNote->registerXPathNamespace("a", Pptx::SCHEMA_DRAWINGML);
        ...
        // Store filename
        $this->addField(Lucene\Field::Text('filename', $fileName, 'UTF-8'));

            // Store contents
        if ($storeContent) {
            $this->addField(Lucene\Field::Text('body', implode(' ', $documentBody), 'UTF-8'));
        } else {
            $this->addField(Lucene\Field::UnStored('body', implode(' ', $documentBody), 'UTF-8'));
        }

        // Store meta data properties
        foreach ($coreProperties as $key => $value)
        {
            $this->addField(Lucene\Field::Text($key, $value, 'UTF-8'));
        }

        // Store title (if not present in meta data)
        if (!isset($coreProperties['title']))
        {
            $this->addField(Lucene\Field::Text('title', $fileName, 'UTF-8'));
        }
    }
    ...
    {
        return new Pptx($fileName, $storeContent);
    }
}
